...
void AudioManager::onUserSpeakerChanged(const std::string& accountId, bool mute, const std::list<std::string>& nickname) {
    QStringList nickNameTmp;
    if ((!mute && accountId == AuthManager::getInstance()->authAccountId().toStdString()) ||
        (accountId != AuthManager::getInstance()->authAccountId().toStdString() && NEMeeting::DEVICE_ENABLED == m_localAudioStatus)) {
...
void AudioManager::muteLocalAudio(bool mute) {
    ...
    m_audioController->muteMyAudio(mute, [=](uint32_t errorCode, const std::string& errorMessage) {
        YXLOG(Info) << "muteMyAudio, mute: " << mute << ", errorCode " << errorCode << ", errorMessage: " << errorMessage << YXLOGEnd;
        if (errorCode != 0) {
...
void AudioManager::setLocalAudioStatus(const int& localAudioStatus) {
    if (m_localAudioStatus != localAudioStatus) {
        ...
        YXLOG(Info) << "m_localAudioStatus changed:" << m_localAudioStatus << ", localAudioStatus: " << localAudioStatus << YXLOGEnd;
