...
const AddressBook: React.FC<SIPBatchCallProps> = ({
  ...
  const searchName = debounce((name: string, page: number) => {
    ...
    if (meetingContactsService) {
      meetingContactsService
        ?.searchContactListByName(name, pageSize, page)
        .then((res) => {
          setConnectMembers(res.data)
        })
    } else {
      neMeeting
        ?.searchAccount({
    ...
    function handleScroll() {
      if (
        ...
        meetingContactsService
          ...
          .then((res) => {
            setConnectMembers([...connectMembers, ...res.data])
            currentPageRef.current += 1
          })
          .finally(() => {
  ...
  }, [connectMembers, meetingContactsService])
  return (
