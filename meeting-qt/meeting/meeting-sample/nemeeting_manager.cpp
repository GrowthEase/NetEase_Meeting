...
void NEMeetingManager::invokeStart(const QJsonObject& object) {
    ...
    if (object.contains("enableEncryption")) {
        enableEncryption = object["enableEncryption"].toBool();
    }
    if (object.contains("encryptionKey")) {
        encryptionKey = object["encryptionKey"].toString();
    }
    ...
    if (ipcMeetingService) {
        ...
        if (enableEncryption) {
            params.encryptionConfig.enable = enableEncryption;
...
void NEMeetingManager::getMeetingInfo() {
    ...
    if (ipcMeetingService) {
        ipcMeetingService->getCurrentMeetingInfo([this](NEErrorCode errorCode, const std::string& errorMessage, const NEMeetingInfo& meetingInfo) {
            if (errorCode == ERROR_CODE_SUCCESS) {
                QJsonObject obj;
                obj["meetingUniqueId"] = meetingInfo.meetingId;
                obj["meetingId"] = QString::fromStdString(meetingInfo.meetingNum);
...
void NEMeetingManager::getHistoryMeetingItem() {
    auto ipcSettingsService = NEMeetingKit::getInstance()->getSettingsService();
    if (ipcSettingsService) {
        ipcSettingsService->getHistoryMeetingItem([this](NEErrorCode errorCode, const std::string& errorMessage,
                                                         const std::list<NEHistoryMeetingItem> listItem) {
            if (errorCode == ERROR_CODE_SUCCESS) {
                if (!listItem.empty()) {
                    auto item = listItem.front();
                    emit getHistoryMeetingInfo(item.meetingId, QString::fromStdString(item.meetingNum), QString::fromStdString(item.shortMeetingNum),
                                               QString::fromStdString(item.subject), QString::fromStdString(item.password),
                                               QString::fromStdString(item.nickname), QString::fromStdString(item.sipId));
                } else {
...
void NEMeetingManager::getPersonalMeetingId() {
    NEMeetingKit::getInstance()->getAccountService()->getPersonalMeetingNumber(
