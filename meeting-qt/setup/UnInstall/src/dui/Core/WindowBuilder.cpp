...
Box* WindowBuilder::Create(STRINGorID xml, CreateControlCallback pCallback, Window* pManager, Box* pParent, Box* userDefinedBox) {
    //ԴIDΪ0-65535ֽڣַָΪ4ֽ
    //ַ<ͷΪXMLַΪXMLļ
    ...
    if (HIWORD(xml.m_lpstr) != NULL) {
        if (*(xml.m_lpstr) == _T('<')) {
            ...
            if (hGlobal) {
                ...
                GlobalFree(hGlobal);
                if (!m_xml.Load(string_resourse.c_str()))
                    return NULL;
            } else {
...
Box* WindowBuilder::Create(CreateControlCallback pCallback, Window* pManager, Box* pParent, Box* userDefinedBox) {
        ...
        if (pstrClass == _T("Global")) {
            ...
            if (pManager->GetHWND()) {
                ...
                for (int i = 0; i < nAttributes; i++) {
                    ...
                    if (pstrName == _T("size")) {
                        LPTSTR pstr = NULL;
                        int cx = _tcstol(pstrValue.c_str(), &pstr, 10);
                        ASSERT(pstr);
                        ...
                        UiRect rcSizeBox;
                        LPTSTR pstr = NULL;
                        rcSizeBox.left = _tcstol(pstrValue.c_str(), &pstr, 10);
                        ASSERT(pstr);
                        ...
                        UiRect rcCaption;
                        LPTSTR pstr = NULL;
                        rcCaption.left = _tcstol(pstrValue.c_str(), &pstr, 10);
                        ASSERT(pstr);
                        ...
                        UiRect rc;
                        LPTSTR pstr = NULL;
                        rc.left = _tcstol(pstrValue.c_str(), &pstr, 10);
                        ASSERT(pstr);
        ...
        if (pstrClass == _T("Global")) {
            ...
            for (CMarkupNode node = root.GetChild(); node.IsValid(); node = node.GetSibling()) {
                ...
                if (pstrClass == _T("Class")) {
                    ...
                    if (!pControlName.empty()) {
                        ASSERT(GlobalManager::GetClassAttributes(pControlName).empty());  //еClassȫֵ
    ...
    for (CMarkupNode node = root.GetChild(); node.IsValid(); node = node.GetSibling()) {
        ...
        if (pstrClass == _T("Image") || pstrClass == _T("Font") || pstrClass == _T("Class") || pstrClass == _T("TextColor")) {
        } else {
            if (!userDefinedBox) {
                ...
                for (int i = 0; i < nAttributes; i++) {
                    ASSERT(i == 0 || _tcscmp(node.GetAttributeName(i), _T("class")) != 0);  // classǵһ
...
Control* WindowBuilder::GetUiLibControl(const std::wstring& pstrClass) {
    ...
    switch (cchLen) {
        case 3:
            if (pstrClass == DUI_CTR_BOX)
                pControl = new Box;
            break;
        case 4:
            if (pstrClass == DUI_CTR_HBOX)
                pControl = new HBox;
            else if (pstrClass == DUI_CTR_VBOX)
                pControl = new VBox;
            break;
        case 5:
            if (pstrClass == DUI_CTR_COMBO)
                pControl = new Combo;
            else if (pstrClass == DUI_CTR_LABEL)
                pControl = new Label;
            break;
        case 6:
            if (pstrClass == DUI_CTR_BUTTON)
                pControl = new Button;
            else if (pstrClass == DUI_CTR_OPTION)
                pControl = new Option;
            else if (pstrClass == DUI_CTR_SLIDER)
                pControl = new Slider;
            else if (pstrClass == DUI_CTR_TABBOX)
                pControl = new TabBox;
            break;
        case 7:
            if (pstrClass == DUI_CTR_CONTROL)
                pControl = new Control;
            else if (pstrClass == DUI_CTR_TILEBOX)
                pControl = new TileBox;
            // else if( pstrClass == DUI_CTR_ACTIVEX )           pControl = new ActiveX;
            break;
        case 8:
            if (pstrClass == DUI_CTR_PROGRESS)
                pControl = new Progress;
            else if (pstrClass == DUI_CTR_RICHEDIT)
                pControl = new RichEdit;
            else if (pstrClass == DUI_CTR_CHECKBOX)
                pControl = new CheckBox;
            // else if( pstrClass == DUI_CTR_DATETIME )		  pControl = new DateTime;
            else if (pstrClass == DUI_CTR_TREEVIEW)
                pControl = new TreeView;
            else if (pstrClass == DUI_CTR_TREENODE)
                pControl = new TreeNode;
            else if (pstrClass == DUI_CTR_HLISTBOX)
                pControl = new ListBox(new HLayout, new Facade);
            else if (pstrClass == DUI_CTR_VLISTBOX)
                pControl = new ListBox(new VLayout, new Facade);
            else if (pstrClass == DUI_CTR_CHILDBOX)
                pControl = new ChildBox;
            else if (pstrClass == DUI_CTR_LABELBOX)
                pControl = new LabelBox;
            break;
        case 9:
            if (pstrClass == DUI_CTR_SCROLLBAR)
                pControl = new ScrollBar;
            else if (pstrClass == DUI_CTR_BUTTONBOX)
                pControl = new ButtonBox;
            else if (pstrClass == DUI_CTR_OPTIONBOX)
                pControl = new OptionBox;
            break;
        case 10:
            // if( pstrClass == DUI_CTR_WEBBROWSER )			  pControl = new WebBrowser;
            break;
        case 11:
            if (pstrClass == DUI_CTR_TILELISTBOX)
                pControl = new ListBox(new TileLayout, new Facade);
            else if (pstrClass == DUI_CTR_CHECKBOXBOX)
                pControl = new CheckBoxBox;
            break;
        case 14:
            break;
        case 15:
            break;
        case 16:
            if (pstrClass == DUI_CTR_LISTLABELELEMENT)
                pControl = new ListLabelElement;
            break;
        case 20:
            if (pstrClass == DUI_CTR_LISTCONTAINERELEMENT)
