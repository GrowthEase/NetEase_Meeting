...
FrontPageForm {
    ...
    function joinmeeting() {
        meetingManager.invokeJoin(meetingManager.personalMeetingId, authManager.appUserNick, checkOpenMicrophone.checked, checkOpenCamera.checked);
    ...
    function resumemeeting() {
        ...
        meetingManager.invokeJoin(idProperty.resumemeetingId, authManager.appUserNick, micStatus, cameraStatus);
    ...
    textMeetingId.validator: RegularExpressionValidator {
    ...
    Component.onCompleted: {
        ...
        if (sharedMeetingId !== '') {
            textMeetingId.text = sharedMeetingId.substring(0, 3) + "-" + sharedMeetingId.substring(3, 6) + "-" + sharedMeetingId.substring(6);
        ...
        Qt.callLater(function () {
                ...
                caption.schedule.visible = Qt.binding(function () {
                        return scheduleList.count > 0;
                    });
    ...
    buttonSubmit.onClicked: {
        if (checkMeetingPwd.checked && inputMeetingPwd.text.trim().length !== 6) {
        ...
        if (popupMode === FrontPage.CreateMode) {
            ...
            meetingManager.invokeStart(checkUsePersonalId.checked ? meetingManager.personalMeetingId : "", authManager.appUserNick, inputMeetingPwd.text, checkOpenMicrophone.checked, checkOpenCamera.checked, meetingManager.isSupportRecord);
        } else {
            statisticsManager.meetingStatistics("meeting_join", "meeting");
            meetingManager.invokeJoin(textMeetingId.text.split("-").join("").split(" ").join(""), authManager.appUserNick, checkOpenMicrophone.checked, checkOpenCamera.checked);
        }
    }
    checkMeetingPwd.onToggled: {
        ...
        if (checkMeetingPwd.checked) {
            ...
            if (inputMeetingPwd.text.trim().length === 0) {
                inputMeetingPwd.text = ('000000' + Math.floor(Math.random() * 999999)).slice(-6);
            }
    ...
    Connections {
        ...
        onAvatarClicked: {
            const popupPosition = caption.avatar.mapToGlobal(-profile.width + caption.avatar.width + 20 /* shadow size */
    ...
    Connections {
        ...
        onCheckUpdateSignal: {
            ...
            if (0 !== updateIgnore && updateIgnore <= clientUpdater.getLatestVersion())
                return;
            if (resultCode !== 200)
                return;
    ...
    Connections {
        ...
        onJoinSignal: {
            ...
            default:
                if (popupWindow.visible) {
                    messageTip.error(errorMessage !== '' ? errorMessage : qsTr('Failed to join meeting'));
                } else {
        ...
        onKickOut: {
            ...
            if (!authManager.resetPasswordFlag) {
                message.warning(qsTr('You have been kickout by muti client.'));
                authManager.logout(false);
                updateEnable = true;
                pageLoader.setSource(Qt.resolvedUrl('qrc:/qml/HomePage.qml'));
            } else {
        ...
        onResumeMeetingSignal: {
            ...
            customDialog.description = qsTr('It was detected that you exited abnormally last time, do you want to resume the meeting?');
        ...
        onStartSignal: {
            ...
            case MeetingStatus.ERROR_CODE_FAILED:
                if (popupWindow.visible) {
                    messageTip.error(errorMessage !== '' ? errorMessage : qsTr('Failed to start meeting'));
                } else {
                    message.error(errorMessage !== '' ? errorMessage : qsTr('Failed to start meeting'));
                }
                break;
            default:
                console.info('Start meeting error code: ' + errorCode)
    ...
    Timer {
        ...
        onTriggered: {
            if (feedbackManager.needsFeedback(meetingManager.lastMeetingDuration))
                npsWindow.showWindow();
        }
    }
    Timer {
