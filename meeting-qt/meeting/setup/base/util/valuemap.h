...
public:
    // 参数的访问限制
    enum { access_read = 0x01, access_write = 0x02 };
...
private:
    ...
    struct NumberToString : public VToString {
        ...
        NumberToString(const char* format)
            : format_(format) {}
        ...
        virtual bool to_value(const char* s, void* v) { return sscanf(s, format_, v) == 1; }
    };
...
public:
    ...
    void bind(const std::string& name, void* ptr, Type type, uint8_t access = access_read) {
        assert(ptr);
        assert(type >= t_char && type < types_num);
        items_[name] = Item(type, ptr, access);
    }
    void bind(const char* name, void* ptr, Type type, uint8_t access = access_read) { bind(std::string(name), ptr, type, access); }
    void unbind(const std::string& name) {
        Map::iterator it = items_.find(name);
        if (it != items_.end())
            items_.erase(it);
    }
    void unbind(const char* name) { unbind(std::string(name)); }
    ...
    std::string get(const char* name) const { return get(std::string(name)); }
    ...
    bool set(const char* name, const char* value) { return set(std::string(name), value); }
