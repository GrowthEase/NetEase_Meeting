    ...
    GridView {
        ...
        delegate: Item {
            ...
            ImageTipButton {
                anchors.fill: parent
                itemIcon: {
                    let checkedIndex = model.itemCheckedIndex
                    if (MoreItemEnum.WhiteboardMenuId === model.itemIndex) {
                         checkedIndex = (whiteboardManager.whiteboardSharing && whiteboardManager.whiteboardSharerAccountId === authManager.authAccountId) ?  2 : 1
                    }
                    return getItemImage((1 === checkedIndex) ? model.itemImage : model.itemImage2)
                }
                itemText:  {
                    let checkedIndex = model.itemCheckedIndex
                    if (MoreItemEnum.WhiteboardMenuId === model.itemIndex) {
                    ...
                    return (1 === checkedIndex) ? model.itemTitle : model.itemTitle2
                }
