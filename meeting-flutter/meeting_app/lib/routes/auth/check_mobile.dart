  ...
  Widget getSubject() {
    return Column(
      ...
      children: <Widget>[
        ...
        buildInputBox(),
        buildNextBtn(),
        if (SceneType.register == authModel.sceneType) protocolTips()
      ],
    );
  }
    ...
    if (SceneType.login == authModel.sceneType) {
      ...
      lifecycleExecuteUI(result).then((result) {
        if (result?.code == HttpCode.success) {
          ...
          if (HttpCode.phoneNotRegister == result?.code) {
            ErrorUtil.showError(context, Strings.mobileNotRegister);
            ...
            ErrorUtil.showError(context, Strings.verifyCodeErrorTip);
          } else {
            ErrorUtil.showError(context, HttpCode.getMsg(result?.msg));
          }
        }
      });
    } else if (SceneType.register == authModel.sceneType ||
        SceneType.resetPwd == authModel.sceneType) {
      var loginResult = authRepo.verifyAuthCode(
          authModel.mobile!, authModel.verifyCode!, authModel.sceneType!);
      lifecycleExecuteUI(loginResult).then((result) {
        if (result == null) return;
        if (result.code == HttpCode.success) {
          authModel.verifyExchangeCode = result.nonNullData;
          switch (authModel.sceneType) {
            case SceneType.register:
              NavUtils.pushNamed(context, RouterName.registerUserInfo,
                  arguments: authModel);
              break;
            case SceneType.resetPwd:
              NavUtils.pushNamed(context, RouterName.passwordVerify,
                  arguments: authModel);
              break;
            default:
          }
