...
bool StringHelper::MBCSToUnicode(const std::string& input, std::wstring& output, int code_page) {
    ...
    if (output.empty())
        return true;
    ::MultiByteToWideChar(code_page, 0, input.c_str(), static_cast<int>(input.size()), &output[0], static_cast<int>(output.size()));
    return true;
}
...
bool StringHelper::UnicodeToMBCS(const wchar_t* input, std::string& output, int code_page) {
    ...
    if (output.empty())
        return true;
    ::WideCharToMultiByte(code_page, 0, input, length - 1, &output[0], static_cast<int>(output.size()), NULL, NULL);
    return true;
}
...
bool StringHelper::UnicodeToMBCS(const std::wstring& input, std::string& output, int code_page) {
    ...
    if (output.empty())
        return true;
    ::WideCharToMultiByte(code_page, 0, input.c_str(), static_cast<int>(input.size()), &output[0], static_cast<int>(output.size()), NULL, NULL);
    return true;
}
