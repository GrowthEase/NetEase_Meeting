        ...
        if (IsEnabled()) {
            if (m_bHorizontal) {
                if (event.ptMouse.x >= m_rcItem.right - m_szThumb.cx / 2)
                    ...
                    m_nValue = m_nMin + double((m_nMax - m_nMin) * (event.ptMouse.x - m_rcItem.left - m_szThumb.cx / 2)) /
                                            (m_rcItem.right - m_rcItem.left - m_szThumb.cx);
            } else {
                if (event.ptMouse.y >= m_rcItem.bottom - m_szThumb.cy / 2)
                    ...
                    m_nValue = m_nMin + double((m_nMax - m_nMin) * (m_rcItem.bottom - event.ptMouse.y - m_szThumb.cy / 2)) /
                                            (m_rcItem.bottom - m_rcItem.top - m_szThumb.cy);
            }
...
void Slider::SetAttribute(const std::wstring& pstrName, const std::wstring& pstrValue) {
    if (pstrName == _T("thumbnormalimage"))
        ...
        CSize szXY;
        LPTSTR pstr = NULL;
        szXY.cx = _tcstol(pstrValue.c_str(), &pstr, 10);
        ASSERT(pstr);
        ...
        Progress::SetAttribute(pstrName, pstrValue);
}
