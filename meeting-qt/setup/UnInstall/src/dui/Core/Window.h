...
class UILIB_API Window : public nbase::SupportWeakCallback, public nbase::SupportDestroyCallback {
public:
    Window();
    ~Window();

    UI_FORBID_COPY(Window)
...
public:
    ...
    UiRect GetPos(bool bContainShadow = false) const;                                                      // bContainShadowΪfalseʾֵӰ
    void SetPos(const UiRect& rc, UINT uFlags, HWND hWndInsertAfter = NULL, bool bContainShadow = false);  // bContainShadowΪfalseʾrcӰ
    CSize GetMinInfo(bool bContainShadow = false) const;                                                   // bContainShadowΪfalseʾֵӰ
    void SetMinInfo(int cx, int cy, bool bContainShadow = false);                                          // bContainShadowΪfalseʾcx cyӰ
    CSize GetMaxInfo(bool bContainShadow = false) const;                                                   // bContainShadowΪfalseʾֵӰ
    void SetMaxInfo(int cx, int cy, bool bContainShadow = false);                                          // bContainShadowΪfalseʾcx cyӰ
    CSize GetInitSize(bool bContainShadow = false) const;                                                  // bContainShadowΪfalseʾֵӰ
    void SetInitSize(int cx, int cy, bool bContainShadow = false);                                         // bContainShadowΪfalseʾcx cyӰ
    ...
    LPBYTE GetBits() const { return m_pBmpBackgroundBits; }

    Control* GetNewHover() { return m_pNewHover; }

    POINT GetLastMousePos() const { return m_ptLastMousePos; }

    bool GetCanvasTransparent() const { return m_bIsCanvasTransparent; }
    ...
    double GetHeightPercent() const { return m_heightPercent; }
    void SetHeightPercent(double heightPercent) { m_heightPercent = heightPercent; }

    void SetTextId(const std::wstring& textId) { ::SetWindowText(m_hWnd, MutiLanSupport::GetInstance()->GetStringViaID(textId).c_str()); }

    void AttachWindowClose(const EventCallback& callback) { OnEvent[EventType::WINDOWCLOSE] += callback; }
...
private:
    WNDPROC m_OldWndProc = ::DefWindowProc;
    bool m_bSubclassed = false;

    HDC m_hDcPaint = nullptr;
    HDC m_hDcBackground = nullptr;
    HBITMAP m_hbmpBackground = nullptr;
    LPBYTE m_pBmpBackgroundBits = nullptr;
    HWND m_hwndTooltip = nullptr;
    TOOLINFO m_ToolTip;

    Control* m_pFocus = nullptr;
    Control* m_pNewHover = nullptr;
    Control* m_pEventHover = nullptr;
    Control* m_pEventClick = nullptr;
    Control* m_pEventKey = nullptr;

    CPoint m_ptLastMousePos{-1, -1};
    CSize m_szMinWindow;
    CSize m_szMaxWindow;
    CSize m_szInitWindowSize;
    UiRect m_rcMaximizeInfo{0, 0, 0, 0};
    UiRect m_rcSizeBox;
    UiRect m_rcCustomShadow{0, 0, 0, 0};
    CSize m_szRoundCorner;
    UiRect m_rcCaption;
    UINT m_uTimerID = 0x1000;
    bool m_bFirstLayout = true;
    bool m_bIsArranged = false;
    bool m_bFocusNeeded = false;
    bool m_bMouseTracking = false;
    bool m_bMouseCapture = false;
    bool m_bIsLayeredWindow = true;
    bool m_bIsCanvasTransparent = false;
    ...
    std::wstring m_pStrWindowResourcePath;  //ÿڵԴ·,GetSkinFolder()
    std::vector<ITranslateAccelerator*> m_aTranslateAccelerator;
    double m_heightPercent = 0;
...
protected:
    ...
    private:
        UiRect m_rcShadowLength{14, 14, 14, 14};
