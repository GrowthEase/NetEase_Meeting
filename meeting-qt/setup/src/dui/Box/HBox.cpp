...
namespace ui
{
	...
	CSize HLayout::ArrangeChild(const std::vector<Control*>& m_items, UiRect rc)
	{
		...
		CSize size = {cxNeeded, szAvailable.cy};
		return size;
	}

	CSize HLayout::AjustSizeByChild(const std::vector<Control*>& m_items, CSize szAvailable)
	{
		CSize totalSize;
		CSize itemSize;
		...
		{
			pChildControl = (Control*)m_items[it];
