...
#include "global/client_global.h"
...
void NEAuthServiceIMP::OnPack(int cid, const std::string& data, uint64_t sn) {
    LOG_IPCCLIENT_INFO(std::string("OnPack, cid: ").append(std::to_string(cid)).append(", sn: ").append(std::to_string(sn)));
    switch (cid) {
        case AuthCID::AuthCID_LoginWithNEMeeting: {
            ...
            if (request.Parse(data)) {
                ...
                LOG_IPCCLIENT_INFO("request.Parse failed.");
                LoginWithNEMeetingResponse response;
                response.error_code_ = MEETING_ERROR_FAILED_PARAM_ERROR;
                response.error_msg_ = "invalid param";
                SendData(AuthCID::AuthCID_LoginWithNEMeeting_CB, response, sn);
            }
        } break;
        ...
        case AuthCID::AuthCID_Login: {
            ...
            if (request.Parse(data)) {
                login(request.appkey_, request.account_, request.password_,
