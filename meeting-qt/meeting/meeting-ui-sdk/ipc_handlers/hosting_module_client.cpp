...
HostingModuleClient::HostingModuleClient(QObject* parent)
    ...
    m_strLogPath = appDataDir + "meeting_ui_" + QDateTime::currentDateTime().toString("yyyyMMddhh-mm-ss").append("_log.txt");
}
...
void HostingModuleClient::setLanguage(int language) {
    if (meeting_sdk_proc_handler_) {
        meeting_sdk_proc_handler_->onSwitchLanguage((NS_I_NEM_SDK::NEMeetingLanguage)language, [=](int code, const std::string& msg) {});
    }
}
