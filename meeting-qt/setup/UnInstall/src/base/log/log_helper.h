...
BASE_EXPORT void DebugCheck(const char* expression, const char* file, int line, bool condition);

#define LOG_IS_ON(level) DEFLOGALLOW(nbase::LogInterface::##level)
#define LAZY_LOG(condition, msg) (!condition) ? (void)0 : msg
#define LOG_MSG(level) nbase::LogMessage(nbase::LogInterface::##level, __FILE__, __LINE__)
...
#ifndef NDEBUG
#define DCHECK(x) nbase::DebugCheck(#x, __FILE__, __LINE__, !!(x));
#else
#define DCHECK(x) true ? (void)0 : nbase::DebugCheck(#x, __FILE__, __LINE__, !!(x));
#endif  // NDEBUG
