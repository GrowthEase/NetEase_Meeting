...
std::wstring CSetupData::GetDefaultInstallDir() {
    ...
    return GetFullFolderPath($PROGRAMFILES _T("Netease\\NIM\\"));
}
...
//获取批量删除文件信息
int CSetupData::GetDelFileInfoListNum(int step) {
    static int del_step = 0;
    if (del_step != step) {
        del_step = step;
        m_strAryDleFileList.clear();
    }
    if (m_strAryDleFileList.size() == 0) {
        ...
        switch (step) {
            ...
            case 2:
                //ɾû
                _DelFile($LOCALAPPDATA _T("\\") PRODUCT_USER_DATA _T("*.*"));
                _DelFile($LOCALAPPDATA _T("\\") PRODUCT_USER_DATA);
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localLastConferenceId");
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localLastChannelId");
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localCameraStatus");
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localMicStatus");
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localShowtime");
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localUserId");
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localAuthToken");
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localUserNickname");
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localAnonCameraStatus");
                DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localAnonMicStatus");
                break;
        }
    }
...
int CSetupData::GetDelRegInfoListNum() {
    if (m_vecDelRegInfoList.size() == 0) {
        ...
        _DelRegKey(HKLM, PRODUCT_DIR_REGKEY);
        _DelRegKey(HKLM, PRODUCT_REGKEY);
        //-------------------------------------------------
    }
...
void CSetupData::CreateLnkList() {
    ...
    _Create_Lnk($INSTDIR UNINSTALL_NAME, $SMPROGRAMS PRODUCT_NAME _T("\\ж") PRODUCT_NAME _T(".lnk"), $INSTDIR UNINSTALL_NAME);
