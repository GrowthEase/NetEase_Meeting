...
// MessageLoop.
class BASE_EXPORT MessageLoopProxy : public ThreadSafeRefCountT<MessageLoopProxy, MessageLoopProxyTraits> {
public:
    ...
    virtual bool PostNonNestableDelayedTask(const StdClosure& task, TimeDelta delay);
...
private:
    ...
    public:
        ...
        void Run() {
            auto ret = std_task_();
            origin_loop_->PostTask(std::bind(&PostTaskAndReplyRelay::RunReplyAndSelfDestructWithParam<decltype(ret)>, this, ret));
        }
...
void MessageLoopProxy::PostTaskAndReplyRelay<void(), void()>::Run() {
    std_task_();
    origin_loop_->PostTask(std::bind(&PostTaskAndReplyRelay::RunReplyAndSelfDestruct, this));
}
