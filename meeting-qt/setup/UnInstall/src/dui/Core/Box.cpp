...
bool Layout::SetAttribute(const std::wstring& pstrName, const std::wstring& pstrValue) {
    ...
    if (pstrName == _T("padding")) {
        UiRect rcPadding;
        LPTSTR pstr = NULL;
        rcPadding.left = _tcstol(pstrValue.c_str(), &pstr, 10);
        ASSERT(pstr);
...
CSize Layout::AjustSizeByChild(const std::vector<Control*>& m_items, CSize szAvailable) {
    ...
    for (auto it = m_items.begin(); it != m_items.end(); it++) {
        ...
        if (itemSize.cx < (*it)->GetMinWidth())
            itemSize.cx = (*it)->GetMinWidth();
        if ((*it)->GetMaxWidth() >= 0 && itemSize.cx > (*it)->GetMaxWidth())
            itemSize.cx = (*it)->GetMaxWidth();
        if (itemSize.cy < (*it)->GetMinHeight())
            itemSize.cy = (*it)->GetMinHeight();
        if (itemSize.cy > (*it)->GetMaxHeight())
            itemSize.cy = (*it)->GetMaxHeight();
...
void Box::SetAttribute(const std::wstring& pstrName, const std::wstring& pstrValue) {
    ...
    } else if (pstrName == _T("clip"))
        SetClip(pstrValue == _T("true"));
    else if (pstrName == _T("mousechild"))
        SetMouseChildEnabled(pstrValue == _T("true"));
    else
...
bool Box::Remove(Control* pControl) {
    ...
    for (auto it = m_items.begin(); it != m_items.end(); it++) {
        if (*it == pControl) {
            Arrange();
            if (m_bAutoDestroy) {
                if (m_bDelayedDestroy && m_pWindow)
...
int Box::FindSelectable(int iIndex, bool bForward /*= true*/) const {
    ...
    if (bForward) {
        for (int i = iIndex; i < GetCount(); i++) {
            if (dynamic_cast<IListItem*>(GetItemAt(i)) != NULL && GetItemAt(i)->IsVisible() && GetItemAt(i)->IsEnabled())
                return i;
        }
        return -1;
    } else {
...
void ScrollableBox::SetAttribute(const std::wstring& pstrName, const std::wstring& pstrValue) {
    if (pstrName == _T("vscrollbar")) {
        ...
        LPTSTR pstr = NULL;
        rcScrollbarPadding.left = _tcstol(pstrValue.c_str(), &pstr, 10);
        ASSERT(pstr);
        rcScrollbarPadding.top = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
        rcScrollbarPadding.right = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
        rcScrollbarPadding.bottom = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
        SetScrollBarPadding(rcScrollbarPadding);
    } else if (pstrName == _T("vscrollunit"))
        SetVerScrollUnitPixels(_ttoi(pstrValue.c_str()));
    else if (pstrName == _T("scrollbarfloat"))
        SetScrollBarFloat(pstrValue == _T("true"));
    else if (pstrName == _T("defaultdisplayscrollbar"))
        SetDefaultDisplayScrollbar(pstrValue == _T("true"));
    else if (pstrName == _T("holdend"))
...
bool ScrollableBox::MouseLeave(EventArgs& msg) {
    ...
    if (ret && m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsValid() && m_pVerticalScrollBar->IsEnabled()) {
        if (m_pVerticalScrollBar->GetThumbState() == ControlStateType::NORMAL && m_pVerticalScrollBar->IsAutoHideScroll()) {
...
void ScrollableBox::ProcessScrollBar(UiRect rc, int cxRequired, int cyRequired) {
    ...
    if (m_pHorizontalScrollBar != NULL && m_pHorizontalScrollBar->IsValid()) {
        UiRect rcHorScrollBarPos(rcScrollBarPos.left, rcScrollBarPos.bottom, rcScrollBarPos.right,
    ...
    if (cyScroll <= 0 && !m_bScrollProcess) {
        ...
        UiRect rcVerScrollBarPos(rcScrollBarPos.right - m_pVerticalScrollBar->GetFixedWidth(), rcScrollBarPos.top, rcScrollBarPos.right,
