...
BASE_EXPORT bool FilePathExtension(const PathString& filepath_in, PathString& extension_out);

// Get the directory from the whole filepath
BASE_EXPORT bool FilePathApartDirectory(const PathString& filepath_in, PathString& directory_out);
// Get the filename from the whole filepath
BASE_EXPORT bool FilePathApartFileName(const PathString& filepath_in, PathString& filename_out);

// Parse and extract all components of a path
BASE_EXPORT bool ParsePathComponents(const PathChar* PathChar, std::list<PathString>& components);
...
BASE_EXPORT bool FilePathCompose(const PathString& directory_in, const PathString& filename_in, PathString& filepath_out);
...
BASE_EXPORT bool FilePathIsExist(const PathChar* filepath_in, bool is_directory);
BASE_EXPORT bool FilePathIsExist(const PathString& filepath_in, bool is_directory);
...
BASE_EXPORT int ReadFile(const PathChar* filepath, void* data_out, size_t size);
BASE_EXPORT int ReadFile(const PathString& filepath, void* data_out, size_t size);

// Read the file at |path| into |contents|, returning true on success.
BASE_EXPORT bool ReadFileToString(const PathString& filepath, std::string& contents_out);

// Writes the content of given buffer into the file
BASE_EXPORT int WriteFile(const PathChar* filepath, const void* data, size_t size);
BASE_EXPORT int WriteFile(const PathString& filepath, const std::string& data);

// Copies a single file.
BASE_EXPORT bool CopyFile(const PathString& from_path, const PathString& to_path);
