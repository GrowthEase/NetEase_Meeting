...
class _PackageVersionSettingState
    extends MeetingBaseState<PackageVersionSetting> {
  @override
  void dispose() {
    super.dispose();
  }
  ...
  Widget buildContent() {
    return widget.edition.featureList.isEmpty
        ? Container()
        : buildList(widget.edition.featureList);
  }
  ...
  Widget buildItem(
      {required Feature item, required int index, bool isLast = false}) {
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <
          ...
          Container(
            ...
            child: Text(item.description,
                style: TextStyle(fontSize: 12, color: AppColors.black_333333)),
          ),
          Visibility(
              visible: isLast,
              child: Container(
                color: AppColors.white,
                padding:
                    EdgeInsets.only(left: 20, top: 10, bottom: isLast ? 24 : 0),
                child: Text(widget.edition.extra ?? '',
                    style:
                        TextStyle(fontSize: 12, color: AppColors.black_333333)),
              )),
        ]));
  }
