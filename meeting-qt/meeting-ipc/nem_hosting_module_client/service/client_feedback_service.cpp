...
void NEFeedbackServiceIMP::feedback(const int& type, const std::string& path, bool needAudioDump, const NEFeedbackService::NEFeedbackCallback& cb) {
    if (_ProcHandler() != nullptr) {
        _ProcHandler()->onFeedback(type, path, needAudioDump, cb);
    }
}
    ...
    switch (cid) {
        case FeedbackCID::FeedbackCID_feedback: {
            ...
            if (request.Parse(data)) {
                feedback(request.type_, request.path_, request.needAudioDump_,
                         ToWeakCallback([this, sn](NEErrorCode error_code, const std::string& error_msg, const std::string& url, const int& type) {
