...
public:
    explicit ToggleValue(bool* value)
        : value_(value) {}
    virtual void Run() { *value_ = !*value_; }
...
public:
    explicit SleepSome(int msec)
        : msec_(msec) {}
    virtual void Run() { nbase::Thread::Sleep(msec_); }
...
public:
    ...
    virtual void Init() { event_list_->push_back(THREAD_EVENT_INIT); }

    virtual void Cleanup() { event_list_->push_back(THREAD_EVENT_CLEANUP); }
...
public:
    explicit RegisterDestructionObserver(nbase::MessageLoop::DestructionObserver* observer)
        : observer_(observer) {}
...
TEST(FrameworkThread, CleanUp) {
...

    {
        ...
        t.message_loop()->PostTask(new RegisterDestructionObserver(&loop_destruction_observer));
