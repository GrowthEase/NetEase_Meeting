...
class InterpreterSwitchLangPanel extends StatefulWidget {
  ...
  const InterpreterSwitchLangPanel({
    super.key,
...
class _InterpreterSwitchLangPanelState
  ...
  Widget build(BuildContext context) {
    ...
    return ValueListenableBuilder(
        valueListenable: widget.collapseNotifier,
        builder: (context, collapsed, _) {
          return AnimatedContainer(
            duration: const Duration(milliseconds: 200),
            child: collapsed
                ? _collapsedPanel()
                : _expandedPanel(mySelfInterpreter, speakLang),
          );
        });
  }
  ...
  Widget _collapsedPanel() {
    return GestureDetector(
      onTap: () {
        widget.collapseNotifier.value = false;
      },
      child: Container(
        width: 40,
        height: 40,
  ...
  Widget _expandedPanel(
      NEMeetingInterpreter mySelfInterpreter, String speakLang) {
    final firstLangSelected = mySelfInterpreter.firstLang == speakLang;
    final secondLangSelected = mySelfInterpreter.secondLang == speakLang;
    final mainLangSelected = speakLang == '';
    return ClipRRect(
          ...
          child: DecoratedBox(
            ...
            child: Row(
              ...
              children: [
                ...
                Visibility.maintain(
                  visible: !firstLangSelected && !secondLangSelected,
                  child: Container(
                    width: 1.w,
                    height: 16.h,
                    color: Colors.white,
                ...
                Visibility.maintain(
                  visible: !secondLangSelected && !mainLangSelected,
                  child: Container(
                    width: 1.w,
                    height: 16.h,
                    color: Colors.white,
                ...
                GestureDetector(
                  ...
                  onTap: () {
                    widget.collapseNotifier.value = true;
                  },
                  child: Container(
  ...
  Widget speakLanguageButton(String? languageTag, String speakLang) {
    ...
    return TextButton(
      ...
      style: ButtonStyle(
        fixedSize: WidgetStateProperty.all(Size(80.w, 40.r)),
        ...
        backgroundColor: WidgetStateProperty.all(
          selected ? _UIColors.color_337eff : Colors.transparent,
        ),
        shape: WidgetStateProperty.all(
          StadiumBorder(),
        ),
        foregroundColor: WidgetStateProperty.all(
            selected ? _UIColors.white : _UIColors.color1E1F27),
      ),
    );
  }
...
class MeetingInterpreterCard extends StatelessWidget {
  ...
  Widget _buildLang(String lang, VoidCallback? onTap, bool empty) {
    return GotoTile(
      key: MeetingUIValueKeys.interpretationLang,
      decoration: BoxDecoration(
        color: _UIColors.colorF2F3F5,
        borderRadius: BorderRadius.circular(4),
      ),
...
class InterpreterListController extends ChangeNotifier {
  ...
  void _loadUserAttachment() async {
    ...
    if (usersWithoutAttachment.isNotEmpty) {
      final result =
          await ContactsRepository.getContactsInfo(usersWithoutAttachment);
      if (result.isSuccess()) {
