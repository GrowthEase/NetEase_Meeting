...
void NEMeetingSDKIMP::privateInitialize(int port) {
    LOG_IPCCLIENT_INFO("privateInitialize start.");
...
void NEMeetingSDKIMP::initialize(const NEMeetingKitConfig& config, const NEInitializeCallback& cb) {
    ...
    lhShareMemory = CreateFileMappingA(HANDLE(0xFFFFFFFF), NULL, PAGE_READWRITE, 0, sizeof(decltype(GetCurrentProcessId())), shared_key.c_str());
...
void NEMeetingSDKIMP::onIPCClientInitialize() {
    ...
    if (_ProcHandler() != nullptr) {
        ...
        LOG_IPCCLIENT_INFO("_ProcHandler is nullptr.")
...
void NEMeetingSDKIMP::InvokeInit(int port) {
    ...
    ipc_client_->AttachInit(
        NS_NIPCLIB::Bind(&NEMeetingSDKIMP::OnIPCClientInit, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void NEMeetingSDKIMP::OnIPCClientInit(bool ret, const std::string& host, int port) {
    LOG_IPCCLIENT_INFO(std::string("OnIPCClientInit, ret: ")
                           .append(ret ? "true" : "false")
                           .append(", host: ")
                           .append(host)
                           .append(", port: ")
                           .append(std::to_string(port)));
    IThread::SetLogger([this](int level, const std::string& strInfo) { WriteLog((LogLevel)level, strInfo); });
    if (ret) {
        auto global = shared_from_this();
...
void NEMeetingSDKIMP::OnReceiveIPCData(const NS_NIPCLIB::IPCData& data) {
    LOG_IPCCLIENT_INFO("OnReceiveIPCData.");
    NEMIPCProtocol protocol;
    if (protocol.Parse(data)) {
        LOG_IPCCLIENT_INFO("OnReceiveIPCData, sid: " + std::to_string(protocol.SID()) + ", cid: " + std::to_string(protocol.CID()) + ", data: ");
        ...
        if (service != nullptr) {
            ...
            LOG_IPCCLIENT_INFO("service not found.");
        }
    }
}
void NEMeetingSDKIMP::OnPack(int cid, const std::string& data, uint64_t sn) {
    LOG_IPCCLIENT_INFO("OnPack, cid: " + std::to_string(cid) + ", data: ");
    switch (cid) {
        ...
        case GlobalCID_ActiveWindow:
            ...
            break;
        case GlobalCID_SwitchLanguage:
            OnPack_SwitchLanguage(data, sn);
            break;
        default:
            break;
    }
...
void NEMeetingSDKIMP::OnPack_QuerySDKVersion(const std::string& data, uint64_t sn) {
    ...
    if (request.Parse(data)) {
        queryKitVersion(ToWeakCallback([this, sn](NEErrorCode error_code, const std::string& error_msg, const std::string& version) {
...
void NEMeetingSDKIMP::OnPack_SwitchLanguage(const std::string& data, uint64_t sn) {
    SettingsIntRequest request;
    if (request.Parse(data)) {
        switchLanguage((NEMeetingLanguage)request.value_, ToWeakCallback([this, sn](NEErrorCode error_code, const std::string& error_msg) {
                           SettingsIntResponse response;
                           response.error_code_ = error_code;
                           response.error_msg_ = error_msg;
                           SendData(GlobalCID::GlobalCID_SwitchLanguage_CB, response, sn);
                       }));
    }
}
