    ...
    if (m_pControl) {
        ...
        if (bFadeHot) {
            if (stateType == ControlStateType::NORMAL || stateType == ControlStateType::HOT) {
                ...
                if (normalImagePath.empty() || hotImagePath.empty() || normalImagePath != hotImagePath ||
                    !m_stateImageMap[ControlStateType::NORMAL].imageAttribute.rcSource.Equal(
                        m_stateImageMap[ControlStateType::HOT].imageAttribute.rcSource)) {
...
void StateColorMap::PaintStatusColor(HDC hDC, UiRect rcPaint, ControlStateType stateType) {
    if (m_pControl) {
        ...
        int hotAlpha = m_pControl->GetHotAlpha();
        if (bFadeHot) {
            if ((stateType == ControlStateType::NORMAL || stateType == ControlStateType::HOT) && !m_stateColorMap[ControlStateType::HOT].empty()) {
