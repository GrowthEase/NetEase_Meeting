    ...
    lifecycleExecuteUI(AccountInfoRepo().getAccountApps()).then((result) {
      ...
      if (result.code == HttpCode.success) {
        ...
        ToastUtils.showToast(
            context, result.msg ?? Strings.networkUnavailableCheck);
      }
    });
  }
            ...
            Expanded(
                child: Container(
                    ...
                    child: Text(
                      item.appName,
                      style: TextStyle(
                          fontSize: 16, color: AppColors.color_222222),
                      textAlign: TextAlign.center,
                    ))),
            Spacer(),
            Container(
                padding: EdgeInsets.only(right: 20),
                child: roundCheckBox(item.appKey)),
          ],
        ));
  }
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          border: Border.all(
              color: currentAppKey == appKey ? Colors.white : Colors.grey,
              width: 0.5), // 边色与边宽度
