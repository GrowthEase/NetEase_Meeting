...
void ProxySocketLayer::DoWhenConnectFailed() {
    DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "Setup proxy error, type=%d, status=%d", proxy_.type_, status_);
...
void ProxySocketLayer::DoWhenConnectted() {
    DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "connect proxy successed, type=%d", proxy_.type_);
...
bool ProxySocketLayer::Http11SendAuthRequest() {
    ...
    if (!proxy_.username_.empty()) {
        ...
        sprintf(buff, "CONNECT %s:%d HTTP/1.1\r\nHost: %s:%d\r\nProxy-Authorization: Basic %s\r\nProxy-Connection: Keep-Alive\r\n\r\n", host_.c_str(),
                port_, host_.c_str(), port_, authInfo.c_str());
    } else {
        sprintf(buff, "CONNECT %s:%d HTTP/1.1\r\nHost: %s:%d\r\nProxy-Connection: Keep-Alive\r\n\r\n", host_.c_str(), port_, host_.c_str(), port_);
    }

    if (0 != tcp_client_->Write(buff, (int)strlen(buff))) {
        DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "Http11SendAuthRequest, send buffer: %s", buff);
...
bool ProxySocketLayer::Http11GetAuthResult() {
    ...
    DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "Http11GetAuthResult, recv buffer: %s", buf);
...
bool ProxySocketLayer::Socks5SendRequest1() {
    DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "Socks5SendRequest1, type=%d, status=%d", proxy_.type_, status_);
...
bool ProxySocketLayer::Socks5GetResult1() {
    DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "Socks5GetResult1, type=%d, status=%d", proxy_.type_, status_);
...
bool ProxySocketLayer::Socks5SendAuthRequst() {
    DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "Socks5SendAuthRequst, type=%d, status=%d", proxy_.type_, status_);
    ...
    auth_req->Ver = 0x01;
    req_len++;
    auth_req->Ulen = (char)proxy_.username_.length();
    req_len++;
    strcpy(auth_req->Name, proxy_.username_.c_str());
    req_len += (int)proxy_.username_.length();
    char pwd_len = (char)proxy_.password_.length();
    memcpy((char*)auth_req + req_len, &pwd_len, 1);
    req_len++;
    strcpy((char*)auth_req + req_len, proxy_.password_.c_str());
    req_len += (int)proxy_.password_.length();
...
bool ProxySocketLayer::Socks5GetAuthResult() {
    DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "Socks5GetAuthResult, type=%d, status=%d", proxy_.type_, status_);
...
bool ProxySocketLayer::Socks5SendRequst2() {
    DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "Socks5SendRequst2, type=%d, status=%d", proxy_.type_, status_);
...
bool ProxySocketLayer::Socks5GetResult2() {
    DEFLOG(nbase::LogInterface::LV_APP, __FILE__, __LINE__, "Socks5GetResult2, type=%d, status=%d", proxy_.type_, status_);
...
bool ProxySocketLayer::Socks4aSendRequest() {
    ...
    memset(request_4a.InvalidIP, 0, 4);
    request_4a.InvalidIP[3] = 0x1;  // first three must be 0x00 and the last one must not be 0x00
