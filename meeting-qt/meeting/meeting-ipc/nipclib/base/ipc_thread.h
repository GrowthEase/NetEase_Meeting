...
private:
    class DelayTask {
        ...
        static constexpr inline int64_t CalcNanoMilli() { return std::nano::den / std::milli::den; }
    ...
    public:
        ...
        inline bool NeedDoTask() { return (std::chrono::steady_clock::now().time_since_epoch().count() / CalcNanoMilli() - run_time_point_) >= 0; }
        inline bool Needrepeat() { return (repeat_times_ > 0); }
        inline void Run() {
            repeat_times_ -= 1;
...
private:
    class STLThread {
    public:
        STLThread(const std::function<void()>& thread_proc)
            : thread_(thread_proc) {}
        void Join() {
            if (thread_.joinable())
                thread_.join();
...
private:
    ...
    public:
        virtual void SetTaskDelegat(ITaskDelegate* delegate) override { delegate_ = delegate; }
    ...
    inline void InsertDelayTask(int time_block, const Task& task, int64_t times = -1) {
        {
            ...
            delay_task.do_repeat_delegate_ =
                std::bind(&IPCThread::RepeatTask, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
...
private:
    ...
    inline void RepeatTask(int time_block, const Task& task, int64_t repeat_times) { InsertDelayTask(time_block, task, repeat_times); }
