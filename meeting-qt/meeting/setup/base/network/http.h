...
public:
    ...
    HttpError HttpGet(const HttpString& url, HttpString& out_content, bool cleanup = true);
    HttpError HttpGet(const HttpString& url, const std::wstring& filepath);
    HttpError HttpGet(const HttpString& url, HttpDataProcessor callback, void* param, bool cleanup = true);

    HttpError HttpPost(const HttpString& url, const HttpString& post, HttpString& out_content, bool cleanup = true);
    HttpError HttpPost(const HttpString& url, const HttpString& post, const std::wstring& filepath);
    HttpError HttpPost(const HttpString& url, const HttpString& post, HttpDataProcessor callback, void* param, bool cleanup = true);

    HttpError HttpsGet(const HttpString& url, bool ssl_peer_verify, const HttpString& ca_path, HttpString& out_content, bool cleanup = true);
    HttpError HttpsGet(const HttpString& url, bool ssl_peer_verify, const HttpString& ca_path, const std::wstring& filepath);
    ...
    HttpError HttpsPost(const HttpString& url,
                        ...
                        bool cleanup = true);
    HttpError HttpsPost(const HttpString& url, bool ssl_peer_verify, const HttpString& ca_path, const HttpString& post, const std::wstring& filepath);
    HttpError HttpsPost(const HttpString& url,
                        bool ssl_peer_verify,
                        const HttpString& ca_path,
                        const HttpString& post,
