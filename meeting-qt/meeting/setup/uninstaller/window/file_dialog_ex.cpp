...
// ʾ_T("Text Files(*.txt)\0*.txt\0ҳļ\0*.htm;*.html\0All Files(*.*)\0*.*\0\0")
...
// param:ַ,key-ַ value-ַ
// ָ
// example:filers[L"ͼļ(*.jpg)"] = L"*.jpg";
...
void CFileDialogEx::AyncShowOpenDirDlg(FileDialogCallback2 file_dialog_callback2) {
    file_dialog_callback2_ = file_dialog_callback2;
...
void CFileDialogEx::SyncShowModal() {
    if (file_dialog_type_ == FDT_OpenFile) {
        ...
        if ((m_stOFN.Flags & OFN_ALLOWMULTISELECT) > 0) {
            ...
            if (file_name.size() == 0) {
                StdClosure closure = std::bind(file_dialog_callback2_, ret, file_directory);
                nbase::ThreadManager::PostTask(threading::kThreadUI, closure);
            } else {
                while (file_name.size() > 0) {
                    std::wstring file_path = file_directory + L"\\" + file_name;
                    if (nbase::FilePathIsExist(file_path, false)) {
                        StdClosure closure = std::bind(file_dialog_callback2_, ret, file_path);
                        nbase::ThreadManager::PostTask(threading::kThreadUI, closure);
                    }
            ...
            StdClosure closure = std::bind(file_dialog_callback2_, ret, GetPathName());
            nbase::ThreadManager::PostTask(threading::kThreadUI, closure);
        }
