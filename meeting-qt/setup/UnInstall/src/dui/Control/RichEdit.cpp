...
BOOL CTxtWinHost::TxShowScrollBar(INT fnBar, BOOL fShow) {
    ASSERT(FALSE);  //ʱע͵֪ɶ   by panqinke 2014.5.6
...
void CTxtWinHost::TxSetCapture(BOOL fCapture) {
    if (fCapture)
        m_re->GetWindow()->SetCapture();
    else
        m_re->GetWindow()->ReleaseCapture();
    fCaptured = fCapture;
}
...
void CTxtWinHost::SetPassword(BOOL bPassword) {
    ...
    pserv->OnTxPropertyBitsChange(TXTBIT_USEPASSWORD, bPassword ? TXTBIT_USEPASSWORD : 0);
}
...
BOOL CTxtWinHost::DoSetCursor(UiRect* prc, POINT* pt) {
    ...
    if (PtInRect(&rc, newPt)) {
        ...
        pserv->OnTxSetCursor(DVASPECT_CONTENT, -1, NULL, NULL, m_re->GetWindow()->GetPaintDC(), NULL, prcClient, newPt.x, newPt.y);
...
void CTxtWinHost::GetControlRect(LPRECT prc) {
    ...
    if (alignType != VerAlignType::TOP) {
        ...
        LONG iHeight = 0;
        SIZEL szExtent = {-1, -1};
        GetTextServices()->TxGetNaturalSize(DVASPECT_CONTENT, m_re->GetWindow()->GetPaintDC(), NULL, NULL, TXTNS_FITTOCONTENT, &szExtent, &iWidth,
                                            &iHeight);
...
HRESULT RichEdit::TxSendMessage(UINT msg, WPARAM wparam, LPARAM lparam, LRESULT* plresult) const {
    if (m_pTwh) {
        ...
        if (msg == WM_KEYDOWN && TCHAR(wparam) == VK_RETURN && ::GetAsyncKeyState(VK_SHIFT) >= 0) {
            if (m_bNeedReturnMsg &&
                ((m_bReturnMsgWantCtrl && ::GetAsyncKeyState(VK_CONTROL) < 0) || (!m_bReturnMsgWantCtrl && ::GetAsyncKeyState(VK_CONTROL) >= 0))) {
                if (m_pWindow != NULL)
...
void RichEdit::SetTimer(UINT idTimer, UINT uTimeout) {
    ...
    auto callback = [this, idTimer]() { this->TxSendMessage(WM_TIMER, idTimer, 0, 0); };
...
bool RichEdit::SetDropAcceptFile(bool bAccept) {
    LRESULT lResult;
    TxSendMessage(EM_SETEVENTMASK, 0,
                  ENM_DROPFILES | ENM_LINK,  // ENM_CHANGE| ENM_CORRECTTEXT | ENM_DRAGDROPDONE | ENM_DROPFILES | ENM_IMECHANGE | ENM_LINK |
...
CSize RichEdit::EstimateSize(CSize szAvailable) {
    ...
    if (size.cx == DUI_LENGTH_AUTO || size.cy == DUI_LENGTH_AUTO) {
        ...
        SIZEL szExtent = {-1, -1};
        m_pTwh->GetTextServices()->TxGetNaturalSize(DVASPECT_CONTENT, GetWindow()->GetPaintDC(), NULL, NULL, TXTNS_FITTOCONTENT, &szExtent, &iWidth,
                                                    &iHeight);
...
void RichEdit::SetPos(UiRect rc) {
    ...
    if (m_pTwh) {
        ...
        if (bVScrollBarVisiable && (!m_pVerticalScrollBar->IsValid() || m_bVScrollBarFixing)) {
            ...
            LONG lHeight = 0;
            SIZEL szExtent = {-1, -1};
            m_pTwh->GetTextServices()->TxGetNaturalSize(DVASPECT_CONTENT, GetWindow()->GetPaintDC(), NULL, NULL, TXTNS_FITTOCONTENT, &szExtent,
                                                        &lWidth, &lHeight);
...
CSize RichEdit::GetNaturalSize(LONG width, LONG height) {
    ...
    if (m_pTwh) {
        m_pTwh->GetTextServices()->TxGetNaturalSize(DVASPECT_CONTENT, GetWindow()->GetPaintDC(), NULL, NULL, TXTNS_FITTOCONTENT, &szExtent, &lWidth,
                                                    &lHeight);
    }
