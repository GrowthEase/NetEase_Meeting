...
const static char PC1_Table[56] = {57, 49, 41, 33, 25, 17, 9,  1, 58, 50, 42, 34, 26, 18, 10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36,
                                   63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22, 14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4};
// permuted choice key (PC2)
const static char PC2_Table[48] = {14, 17, 11, 24, 1,  5,  3,  28, 15, 6,  21, 10, 23, 19, 12, 4,  26, 8,  16, 7,  27, 20, 13, 2,
                                   41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32};
// number left rotations of pc1
const static char Shift_Table[16] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
// initial permutation (IP)
const static char IP_Table[64] = {58, 50, 42, 34, 26, 18, 10, 2,  60, 52, 44, 36, 28, 20, 12, 4,  62, 54, 46, 38, 30, 22,
...
const static char E_Table[48] = {32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,  8,  9,  10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
...
const static char S_Box[8][4][16] = {
    ...
    // S7
    4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1, 13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6, 1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6,
...
const static char P_Table[32] = {16, 7, 20, 21, 29, 12, 28, 17, 1,  15, 23, 26, 5,  18, 31, 10,
                                 2,  8, 24, 14, 32, 27, 3,  9,  19, 13, 30, 6,  22, 11, 4,  25};
// final permutation IP^-1
const static char IPR_Table[64] = {40, 8,  48, 16, 56, 24, 64, 32, 39, 7,  47, 15, 55, 23, 63, 31, 38, 6,  46, 14, 54, 22,
                                   62, 30, 37, 5,  45, 13, 53, 21, 61, 29, 36, 4,  44, 12, 52, 20, 60, 28, 35, 3,  43, 11,
                                   51, 19, 59, 27, 34, 2,  42, 10, 50, 18, 58, 26, 33, 1,  41, 9,  49, 17, 57, 25};
std::string yxDES::Decrypt(std::string& src, const char* key, int iMode, int iPKCS) {
    std::string des;
...
std::string yxDES::Encrypt(std::string& src, const char* key, int iMode, int iPKCS) {
    std::string des;
...
int yxDES::ConvertHex2Ciphertext(const char* szCipherInBytes) {
    char* bitsCiphertextAnyLength = new char[data_base_length_ * 4];
    char* hexCiphertextAnyLengthTmp = new char[data_base_length_ * 2];

    int iLen = 0;

    memset(szCiphertextData, 0, data_base_length_);
    memset(hexCiphertextAnyLengthTmp, 0, data_base_length_ * 2);

    iLen = ((strlen(szCipherInBytes) >> 2) + (strlen(szCipherInBytes) % 4 == 0 ? 0 : 1)) << 4;

    memcpy(hexCiphertextAnyLengthTmp, szCipherInBytes, strlen(szCipherInBytes));
    Hex2Bits(hexCiphertextAnyLengthTmp, bitsCiphertextAnyLength, iLen);
    Bits2Bytes(szCiphertextData, bitsCiphertextAnyLength, iLen);

    delete[] bitsCiphertextAnyLength;
    delete[] hexCiphertextAnyLengthTmp;
