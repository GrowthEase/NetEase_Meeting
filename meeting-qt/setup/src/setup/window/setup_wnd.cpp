...
void SetupForm::SetupCheck()
{
	...
	if (!path.empty())
	{
		...
		// 如果指定的目录末尾不是 NIM_MEETING，则自动在路径后面增加 NIM_MEETING，防止安装目录指定为一个层级较高的目录后导致卸载时删除了非安装包以内的文件
			...
			nbase::MakeLowerString(pre_path) != nbase::MakeLowerString(check_path))
		{
			...
			MsgBox()->SetTitle(L"STRING_TIPS")->SetIcon(kMsgIconWarn)
				->SetInfor(L"检测到网易会议已经安装在：", false)->SetPathInfo(pre_path)->SetInfor2(L"是否确认修改安装目录？")
