...
public:
    virtual void onScheduleMeeting(const NS_I_NEM_SDK::NEMeetingItem& item,
                                   const NS_I_NEM_SDK::NEPreMeetingService::NEScheduleMeetingItemCallback& callback) override;
    virtual void onEditMeeting(const NS_I_NEM_SDK::NEMeetingItem& item,
                               const NS_I_NEM_SDK::NEPreMeetingService::NEOperateScheduleMeetingCallback& callback) override;
    virtual void onCancelMeeting(const int64_t& meetingUniqueId,
                                 const NS_I_NEM_SDK::NEPreMeetingService::NEOperateScheduleMeetingCallback& callback) override;
    virtual void onGetMeetingItemById(const int64_t& meetingUniqueId,
                                      const NS_I_NEM_SDK::NEPreMeetingService::NEScheduleMeetingItemCallback& callback) override;
    virtual void onGetMeetingList(std::list<NS_I_NEM_SDK::NEMeetingItemStatus> status,
                                  const NS_I_NEM_SDK::NEPreMeetingService::NEGetMeetingListCallback& callback) override;

public slots:
    void onScheduleOrEditMeeting(int errorCode, const QString& errorMessage, const nem_sdk_interface::NEMeetingItem& info);
    void onQueryMeetingList(int errorCode, const QString& errorMessage, const QList<nem_sdk_interface::NEMeetingItem>& scheduledLists);
