...
Http::Http()
    ...
    if (NULL == connection_) {
        ...
        DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, "nbase::Http: curl_easy_init() error");
    }
}
...
HttpError Http::SetProxy(const Proxy& proxy) {
    ...
    if (!proxy.user.empty()) {
        curl_easy_setopt(connection_, CURLOPT_PROXYUSERNAME, proxy.user.c_str());
        if (!proxy.pass.empty())
            curl_easy_setopt(connection_, CURLOPT_PROXYPASSWORD, proxy.pass.c_str());
    }

    return CURLE_OK;
}

HttpError Http::HttpGet(const HttpString& url, HttpString& out_content, bool cleanup /* = true*/) {
    return HttpGet(url, OnRawDataGotMemory, &out_content, cleanup);
}

HttpError Http::HttpGet(const HttpString& url, const std::wstring& filepath) {
...
HttpError Http::HttpGet(const HttpString& url, HttpDataProcessor callback, void* param, bool cleanup /* = true*/) {
    HttpString ca_path, post;
    return CurlHelper(false, url, false, ca_path, post, callback, param, cleanup);
}

HttpError Http::HttpPost(const HttpString& url, const HttpString& post, HttpString& out_content, bool cleanup /* = true*/) {
    out_content.clear();
    return HttpPost(url, post, OnRawDataGotMemory, &out_content, cleanup);
}

HttpError Http::HttpPost(const HttpString& url, const HttpString& post, const std::wstring& filepath) {
    FILE* file = OpenFile(filepath, L"wb+");
...
HttpError Http::HttpPost(const HttpString& url, const HttpString& post, HttpDataProcessor callback, void* param, bool cleanup /* = true*/) {
    HttpString ca_path;
    return CurlHelper(true, url, false, ca_path, post, callback, param, cleanup);
}

HttpError Http::HttpsGet(const HttpString& url, bool ssl_peer_verify, const HttpString& ca_path, HttpString& out_content, bool cleanup /* = true*/) {
    return HttpsGet(url, ssl_peer_verify, ca_path, OnRawDataGotMemory, &out_content, cleanup);
}

HttpError Http::HttpsGet(const HttpString& url, bool ssl_peer_verify, const HttpString& ca_path, const std::wstring& filepath) {
    FILE* file = OpenFile(filepath, L"wb+");

    last_error_ = HttpsGet(url, ssl_peer_verify, ca_path, OnRawDataGotFile, file);
...
HttpError Http::HttpsGet(const HttpString& url,
                         ...
                         bool cleanup /* = true*/) {
    HttpString post;
    return CurlHelper(false, url, ssl_peer_verify, ca_path, post, callback, param, cleanup);
}
...
HttpError Http::HttpsPost(const HttpString& url,
                          ...
                          bool cleanup /* = true*/) {
    return HttpsPost(url, ssl_peer_verify, ca_path, post, OnRawDataGotMemory, &out_content, cleanup);
}
...
HttpError Http::HttpsPost(const HttpString& url,
    ...
    last_error_ = HttpsPost(url, ssl_peer_verify, ca_path, post, OnRawDataGotFile, file);
...
HttpError Http::HttpsPost(const HttpString& url,
                          ...
                          void* param,
                          bool cleanup /* = true*/) {
    return CurlHelper(true, url, ssl_peer_verify, ca_path, post, callback, param, cleanup);
}

HttpError Http::ErrorHelper(const char* format, HttpError error) {
    DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, format, error);
    Cleanup();
    return last_error_;
}
...
HttpError Http::CurlHelper(bool http_post,
    ...
    if (last_error_ != CURLE_OK) {
        return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_URL error:%d", last_error_);
    }
    ...
    if (last_error_ != CURLE_OK) {
        return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_SSL_VERIFYPEER error:%s", error_buffer);
    }
    ...
    if (!ca_path.empty()) {
        ...
        if (last_error_ != CURLE_OK) {
            return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_CAPATH error:%s", error_buffer);
        }
    }
    ...
    if (headers_ != NULL) {
        ...
        if (last_error_ != CURLE_OK) {
            return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_HTTPHEADER error:%s", error_buffer);
        }
    }
    ...
    if (last_error_ != CURLE_OK) {
        return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_URL error:%s", error_buffer);
    }
    ...
    if (http_post) {
        ...
        if (last_error_ != CURLE_OK) {
            return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_POST error:%s", error_buffer);
        }
        if (!post.empty()) {
            last_error_ = curl_easy_setopt(connection_, CURLOPT_POSTFIELDS, post.c_str());
            if (last_error_ != CURLE_OK) {
                return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_COPYPOSTFIELDS error:%s", error_buffer);
            }
            ...
            if (last_error_ != CURLE_OK) {
                return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_POSTFIELDSIZE error:%s", error_buffer);
            }
        }
    }
    ...
    if (last_error_ != CURLE_OK) {
        return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_FOLLOWLOCATION error:%s", error_buffer);
    }

    // set write function
    last_error_ = curl_easy_setopt(connection_, CURLOPT_WRITEFUNCTION, callback);
    if (last_error_ != CURLE_OK) {
        return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_WRITEFUNCTION error:%s", error_buffer);
    }

    // set write data
    last_error_ = curl_easy_setopt(connection_, CURLOPT_WRITEDATA, param);
    if (last_error_ != CURLE_OK) {
        return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_WRITEDATA error:%s", error_buffer);
    }
    ...
    if (timeout_ > TimeDelta()) {
        ...
        if (last_error_ != CURLE_OK) {
            return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_TIMEOUT error:%s", error_buffer);
        }
    }
    ...
    if (last_error_ != CURLE_OK) {
        return ErrorHelper("nbase::Http: curl_easy_setopt() CURLOPT_COOKIEFILE error:%s", error_buffer);
    }

    // excute
    last_error_ = curl_easy_perform(connection_);
    if (last_error_ != CURLE_OK) {
        return ErrorHelper("nbase::Http: curl_easy_perform() failed error:%s", error_buffer);
    }
