...
struct timespec TimeDelta::ToTimeSpec() const {
    ...
    struct timespec result = {seconds, microseconds * Time::kNanosecondsPerMicrosecond};
    return result;
}
...
static const int64_t kWindowsEpochDeltaMilliseconds = kWindowsEpochDeltaSeconds * Time::kMillisecondsPerSecond;
...
Time::TimeStruct Time::ToTimeStruct(bool is_local) {
    ...
    int64_t milliseconds = (us_ - kWindowsEpochDeltaMicroseconds) / kMicrosecondsPerMillisecond;
...
Time Time::FromTimeStruct(bool is_local, TimeStruct& ts) {
    ...
    if (seconds == -1 && (ts.year() < 1969 || ts.year() > 1970)) {
...
TimeTicks TimeTicks::Now() {
    ...
    absolute_micro = mach_absolute_time() / Time::kNanosecondsPerMicrosecond * timebase_info.numer / timebase_info.denom;
...
#elif (defined(OS_POSIX) && defined(_POSIX_MONOTONIC_CLOCK) && _POSIX_MONOTONIC_CLOCK >= 0) || defined(OS_FREEBSD)
...
TimeTicks TimeTicks::Now() {
    ...
    absolute_micro =
        (static_cast<int64>(ts.tv_sec) * Time::kMicrosecondsPerSecond) + (static_cast<int64>(ts.tv_nsec) / Time::kNanosecondsPerMicrosecond);
...
TimeTicks TimeTicks::Now() {
    ...
    absolute_micro = (static_cast<int64_t>(tv.tv_sec) * Time::kMicrosecondsPerSecond) + (static_cast<int64_t>(tv.tv_usec));
