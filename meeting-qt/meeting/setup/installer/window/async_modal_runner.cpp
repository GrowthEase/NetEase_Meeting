...
AsyncModalRunnerManager* AsyncModalRunnerManager::GetInstance() {
    return nbase::Singleton<AsyncModalRunnerManager>::get();
}
...
void AsyncModalRunnerManager::CancelAllThreads() {
    ...
    std::for_each(threads.begin(), threads.end(), [](std::shared_ptr<AsyncModalRunner> runner) { runner->CancelModalThenExit(); });
    // Then, we wait util all modal runner exited
    std::for_each(threads.begin(), threads.end(), [](std::shared_ptr<AsyncModalRunner> runner) { runner->Close(); });
}

void AsyncModalRunnerManager::OnThreadWillExit(AsyncModalRunner* runner) {
    nbase::ThreadManager::PostTask(threading::kThreadUI, std::bind(&AsyncModalRunnerManager::Deregister, this, runner));
}
