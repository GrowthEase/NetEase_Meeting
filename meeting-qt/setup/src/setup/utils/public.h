...
BOOL DeleteRegValue(HKEY rootKey, const std::wstring& subKey, const std::wstring& keyName) {
    HKEY hKey;
    if (ERROR_SUCCESS == ::RegOpenKey(rootKey, subKey.c_str(), &hKey)) {
        if (ERROR_SUCCESS == ::RegDeleteValue(hKey, keyName.c_str())) {
            RegCloseKey(hKey);
            return TRUE;
        }
        RegCloseKey(hKey);
...
void SafeWow64EnableWow64FsRedirection(BOOL Wow64FsEnableRedirection) {
    ...
    if (kernel) {
        ...
        if (proc) {
            ...
            Wow64EnableWow64FsRedirectionPtr Wow64EnableWow64FsRedirectionFunc = (Wow64EnableWow64FsRedirectionPtr)proc;
...
BOOL CreateLowIntegrityProcess(LPCTSTR pszFile, LPCTSTR pszCommandLine) {
    ...
    if (!CreateProcessAsUser(hNewToken, (LPCWSTR)pszFile, (LPWSTR)pszCommandLine, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
        dwError = GetLastError();
        goto Cleanup;
    }
...
typedef BOOL(WINAPI* F_CreateProcessWithTokenW)(__in HANDLE hToken,
...
//以普通权限启动进程
BOOL CreateProcessLow(TCHAR* lpApplicationName, TCHAR* lpCommandLine, TCHAR* lpDirectory, UINT nShow) {
    ...
    STARTUPINFO si = {sizeof(STARTUPINFO)};
    PROCESS_INFORMATION pi = {0};
    BOOL ret = pfn(hToken, LOGON_WITH_PROFILE, lpApplicationName, lpCommandLine, NORMAL_PRIORITY_CLASS, NULL, lpDirectory, &si, &pi);
    if (ret) {
        CloseHandle(pi.hProcess);
        CloseHandle(pi.hThread);
    }
