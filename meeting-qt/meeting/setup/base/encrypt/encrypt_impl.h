...
public:
    ...
    bool Encrypt(const std::string& sdata, std::string& ddata) { return Encrypt(sdata.data(), sdata.size(), ddata); }
    bool Decrypt(const std::string& sdata, std::string& ddata) { return Decrypt(sdata.data(), sdata.size(), ddata); }
    bool CreateKey(std::string& key1, std::string& key2) {
        // 16 bytes
        for (;;) {
            char buf[20];
...
public:
    ...
    bool Decrypt(const void* sdata, size_t ssize, std::string& ddata) { return false; }
    bool CreateKey(std::string& key1, std::string& key2) { return false; }
    bool SetPaddingMode(int padding_mode) { return false; }
...
public:
    IEncryptRSAImpl()
        ...
        , e_(0)
        , d_(0)
        , padding_mode_(RSA_PKCS1_PADDING) {}
    ...
    bool SetEncryptKey(const std::string& key) {
        ...
        size_t n;
        nbase::Unpack up(key.data(), key.size());
        try {
            // n
            n = up.pop_uint32();
            ptr = (const unsigned char*)up.pop_fetch_ptr(n);
            n_ = BN_bin2bn(ptr, n, n_);
            //    e
            n = up.pop_uint32();
            ptr = (const unsigned char*)up.pop_fetch_ptr(n);
            e_ = BN_bin2bn(ptr, n, e_);
    ...
    bool Encrypt(const void* sdata, size_t ssize, std::string& ddata) {
        ...
        try {
            ...
            for (uint32_t i = 0; i < nBlock; i++) {
                ...
                if ((i + 1) == nBlock && (ssize % block_size != 0) && (RSA_PKCS1_PADDING == padding_mode_ || RSA_PKCS1_OAEP_PADDING == padding_mode_))
                    sz = ssize % block_size;
    ...
    bool CreateKey(std::string& key1, std::string& key2) {
        RSA* rsa = RSA_new();
        BIGNUM* bne = BN_new();
        bool ret = !!BN_set_word(bne, RSA_F4);
        ...
        int n;
        {
            ...
            try {
                // n and e
                n = BN_bn2bin(rsa->n, buf);
                pack.push_uint32(n);
                pack.push(buf, n);
                n = BN_bn2bin(rsa->e, buf);
                pack.push_uint32(n);
                pack.push(buf, n);
            } catch (const std::runtime_error&) {
                return false;
            }
            key1.assign(pack.data(), pack.size());
        }

        {
            ...
            try {
                ...
                pack.push_uint32(n);
                pack.push(buf, n);
                n = BN_bn2bin(rsa->d, buf);
                pack.push_uint32(n);
                pack.push(buf, n);
            } catch (const std::runtime_error&) {
                return false;
            }
            key2.assign(pack.data(), pack.size());
        }
        n_ = rsa->n;
        rsa->n = 0;
        e_ = rsa->e;
...
public:
    ...
    bool Encrypt(std::string& data) { return Encrypt(data.data(), data.size(), data); }
    bool Decrypt(std::string& data) { return Decrypt(data.data(), data.size(), data); }
...
public:
    ...
    bool SetMethod(nbase::EncryptMethod type) {
        ...
        if (it == methods_.end()) {
            nbase::EncryptMethodInterface_var e;
            e = CreateMethod(type);
            if (e.is_nil())
                return false;
            method_ = e;
    ...
    bool AddMethod(nbase::EncryptMethodInterface* method) {
        if (!method)
            return false;
