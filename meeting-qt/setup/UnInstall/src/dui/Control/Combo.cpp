...
void CComboWnd::Init(Combo* pOwner) {
    ...
    UiRect rc = rcOwner;
    rc.top = rc.bottom + 1;          // leftbottomλΪ
    rc.bottom = rc.top + szDrop.cy;  // 㵯ڸ߶
    if (szDrop.cx > 0)
        rc.right = rc.left + szDrop.cx;  // 㵯ڿ
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (uMsg == WM_CREATE) {
        ...
        if (pControl && typeid(*pControl) != typeid(ScrollBar))
...
bool Combo::SelectItem(int iIndex, bool bTakeFocus, bool bTrigger) {
    ...
    if (m_items.size() == 0)
        return false;
    if ((std::size_t)iIndex >= m_items.size())
        iIndex = m_items.size() - 1;
...
bool Combo::SetItemIndex(Control* pControl, std::size_t iIndex) {
    ...
    if (!Box::SetItemIndex(pControl, iIndex))
        return false;
...
bool Combo::AddAt(Control* pControl, std::size_t iIndex) {
    ...
    if (m_iCurSel >= 0 && (std::size_t)m_iCurSel >= iIndex)
        m_iCurSel += 1;
    return true;
}
...
bool Combo::RemoveAt(std::size_t iIndex) {
    ...
    if ((int)iIndex == m_iCurSel && m_iCurSel >= 0) {
        ...
        m_iCurSel = -1;
        SelectItem(FindSelectable(iSel, false));
    } else if (m_iCurSel >= 0 && iIndex < (std::size_t)m_iCurSel)
        m_iCurSel -= 1;
    return true;
}
