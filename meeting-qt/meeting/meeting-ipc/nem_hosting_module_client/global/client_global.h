...
#define LOG_IPCCLIENT(logLevel, log)                                                                                                         \
    {                                                                                                                                        \
        ...
        if (ipcClient) {                                                                                                                     \
            ipcClient->WriteLog(logLevel,                                                                                                    \
                                /*std::string(__FILE__) + " " + */ std::string(__FUNCTION__) + ":" + std::to_string(__LINE__) + "] " + log); \
        }                                                                                                                                    \
    }
...
public:
    virtual void initialize(const NEMeetingKitConfig& config, const NEInitializeCallback& cb) override;
    virtual void unInitialize(const NEUnInitializeCallback& cb) override;
    virtual bool isInitialized() override { return true; };
    virtual void queryKitVersion(const NEQueryKitVersionCallback& cb) override;
    virtual void activeWindow(/*bool bRaise, */ const NEActiveWindowCallback& cb) override;
    void switchLanguage(NEMeetingLanguage language, const NEEmptyCallback& cb = NEEmptyCallback());
    virtual void setSoftwareRender(bool bSoftware, const NEEmptyCallback& cb) override {}
    virtual void isSoftwareRender(const NEBoolCallback& cb) override {}
    virtual void setExceptionHandler(const NEExceptionHandler& handler) override { exception_handler_ = handler; }
    ...
    virtual void attachPrivateInitialize(const std::function<void(bool)>& cb) override { private_initialize_cb_ = cb; }
    virtual void setLogCallBack(const std::function<void(LogLevel level, const std::string&)>& cb) override { log_cb_ = cb; }
    virtual void Exit();
    virtual void OnLoad() override;
    virtual void OnRelease() override;

public:
    NEMeetingKitConfig GetInitConfig() const;
    void WriteLog(LogLevel level, const std::string& strLog);

private:
    void InvokeInit(int port);
    void AttachUnInit(const std::function<void()>& callback) { uninit_callback_ = callback; }
...
private:
    ...
    void OnPack_SwitchLanguage(const std::string& data, uint64_t sn);
...
private:
    ...
    NEMeetingKitConfig global_config_;
