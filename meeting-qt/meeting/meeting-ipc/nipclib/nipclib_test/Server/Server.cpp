...
int main() {
    server.AttachInit([](bool ret, const std::string& host, int port) {
        ...
        text.append(std::to_string(ret)).append(" host:").append(host).append(" port:").append(std::to_string(port)).append(")");
        std::cout << text << std::endl;
    });
    server.AttachReady([]() { std::cout << "read" << std::endl; });
    ...
    server.AttachClientClose([]() { main_thread.PostTask([] { std::cout << "client exit!" << std::endl; }); });
    // timer_thread.Start();
