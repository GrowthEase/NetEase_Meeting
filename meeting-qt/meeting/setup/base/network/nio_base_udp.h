...
public:
    UdpSock(struct event_base* event_base, uint16_t port, const char* host, ProxySet* proxyset = NULL);
    virtual ~UdpSock() {}
...
class UdpIO : public nbase::UdpSock, public nbase::NIOInputAbleInterface, public nbase::RefCount {
    IOClass& io_;

public:
    UdpIO(IOClass& io, struct event_base* event_base, const char* ip, uint16_t port)
        : nbase::UdpSock(event_base, port, ip)
        , io_(io) {
        Bind(this);
    }
    virtual ~UdpIO() {}
    ...
    void OnRead() {
        ...
        for (uint32_t nmax = 100; nmax > 0; --nmax) {  // must setting a max packet count here
            char buff[1024 * 32];
            uint32_t ip;
            uint16_t port;
