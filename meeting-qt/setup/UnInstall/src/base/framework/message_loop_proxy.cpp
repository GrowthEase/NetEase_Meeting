...
bool MessageLoopProxy::PostDelayedTask(const StdClosure& task, TimeDelta delay) {
...
bool MessageLoopProxy::PostNonNestableDelayedTask(const StdClosure& task, TimeDelta delay) {
...
bool MessageLoopProxy::BelongsToCurrentThread() const {
    ...
    return (target_message_loop_ && (MessageLoop::current() == target_message_loop_));
}
...
void MessageLoopProxy::OnDestruct() const {
    ...
    {
        NAutoLock lock(&message_loop_lock_);
        if (target_message_loop_ && (MessageLoop::current() != target_message_loop_)) {
            target_message_loop_->PostNonNestableTask(std::bind(&MessageLoopProxy::DeleteSelf, this));
            delete_later = true;
        }
    }
    if (!delete_later)
...
bool MessageLoopProxy::PostTaskHelper(const StdClosure& task, TimeDelta delay, bool nestable) {
    NAutoLock lock(&message_loop_lock_);
