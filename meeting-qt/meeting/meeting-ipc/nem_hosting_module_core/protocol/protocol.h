...
public:
    NEMIPCProtocolHead(int sid, int cid)
        : sid_(sid)
        , cid_(cid) {
        sn_ = (--g_sn_);
    }
    NEMIPCProtocolHead(int sid, int cid, uint64_t sn)
        : sid_(sid)
        , cid_(cid)
        , sn_(sn) {}
...
public:
    NEMIPCProtocol(int sid, int cid)
        : head_(sid, cid) {}
    NEMIPCProtocol(int sid, int cid, uint64_t sn)
        : head_(sid, cid, sn) {}
    NEMIPCProtocol()
        : head_(0, 0, 0) {}
...
public:
    ...
    int SID() const { return head_.sid_; }
    int CID() const { return head_.cid_; }
    uint64_t SN() const { return head_.sn_; }
    std::string BodyText() const { return body_text_; }

private:
    NEMIPCProtocolHead head_;
    std::string body_text_;
};
enum ServiceID { SID_Global = 0, SID_Auth = 1, SID_Metting = 2, SID_Setting = 3, SID_Account = 4, SID_Feedback = 5, SID_PreMeeting = 6 };
