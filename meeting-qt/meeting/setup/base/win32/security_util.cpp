...
bool IsUserAdmin() {
    ...
    BOOL result =
        ::AllocateAndInitializeSid(&nt_authority, 2, SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0, 0, 0, &administrators_group);
    if (result) {
...
bool IsUserNonElevatedAdmin() {
    ...
    if (::OpenProcessToken(::GetCurrentProcess(), TOKEN_READ, &token)) {
        ...
        DWORD infoLen = 0;
        if (::GetTokenInformation(token.Get(), TokenElevationType, reinterpret_cast<void*>(&elevation_type), sizeof(elevation_type), &infoLen)) {
...
HRESULT GetSidIntegrityLevel(PSID sid, MANDATORY_LEVEL* level) {
    static SID_IDENTIFIER_AUTHORITY mandatory_label_auth = SECURITY_MANDATORY_LABEL_AUTHORITY;
...
HRESULT GetProcessIntegrityLevel(DWORD process_id, MANDATORY_LEVEL* level) {
    ...
    if (process != NULL) {
        ...
        if (::OpenProcessToken(process, TOKEN_QUERY | TOKEN_QUERY_SOURCE, &current_token)) {
            ...
            ::GetTokenInformation(current_token, TokenIntegrityLevel, NULL, 0, &label_size);
            if (label_size && (label = reinterpret_cast<TOKEN_MANDATORY_LABEL*>(::LocalAlloc(LPTR, label_size))) != NULL) {
                if (::GetTokenInformation(current_token, TokenIntegrityLevel, label, label_size, &label_size)) {
