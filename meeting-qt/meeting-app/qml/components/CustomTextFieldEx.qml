...
TextField {
    ...
    leftPadding: !visibleCalendar ? 12 : idCalendarImage.width
                                    + idCalendarImage.anchors.leftMargin + 8
    selectByMouse: true
    ...
    background: Rectangle {
        ...
        color: control.enabled
               && !idQtObject.readOnly ? "transparent" : "#F7F8FA"
        border.width: control.enabled && !idQtObject.readOnly
                      && control.focus ? 2 : 0
        border.color: "#32337eff"
        radius: 2
        Rectangle {
            anchors.fill: parent
            anchors.margins: control.enabled && !idQtObject.readOnly
                             && control.focus ? 2 : 0
            color: parent.color
            radius: 2
            border.width: 1
            border.color: (control.enabled && !idQtObject.readOnly
                           && (control.hovered
                               || control.focus)) ? control.Material.accentColor : "#DCDFE5"
        }
    }
    ...
    QtObject {
        ...
        property bool readOnly: (visibleCalendar || visibleComboBox) && !acceptToolClickOnly
                                && control.readOnly ? false : control.readOnly
    }
    ...
    Image {
        ...
        visible: control.length !== 0 && control.enabled && control.hovered
                 && !control.readOnly && !acceptToolClickOnly
