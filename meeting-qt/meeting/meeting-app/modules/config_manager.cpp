...
void ConfigManager::requestServerAppConfigs() {
    YXLOG(Info) << "requestServerAppConfigs" << YXLOGEnd;
    QString timestamp = ConfigManager::getInstance()->getValue("lastConfigTimestampEx", "").toString();
    if (timestamp.isEmpty()) {
        timestamp = "0";
    }
    nem_auth::GetAppConfigs request(timestamp);
    m_httpManager->getRequest(request, [this](int code, const QJsonObject& response) {
        if (code == 0) {
            QJsonArray configs = response["tips"].toArray();
            if (configs.size() > 0) {
                ...
                m_safeTipJson.insert("url", url);
                m_safeTipJson.insert("type", obj["type"].toInt());
                setNeedSafeTip(true);
            }
            QString lastConfigTimestamp = response["curTime"].toVariant().toString();
            setValue("lastConfigTimestampEx", lastConfigTimestamp);
        }
    });
}
