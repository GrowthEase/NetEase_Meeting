...
void FrameworkThread::StopSoon() {
    ...
    stopping_ = true;
    message_loop_->PostNonNestableTask(std::bind(&FrameworkThread::DoStopSoon, this));
}
...
void FrameworkThread::Run() {
    ...
    SetThreadWasQuitProperly(false);
    {
...
#endif
            // else if (loop_type_ == UIMessageLoop::kUVMessageLoop)
            //	message_loop = new UvMessageLoop;
            else
        ...
        if (loop_type_ != MessageLoop::kCustomMessageLoop)
            ...
            static_cast<CustomMessageLoop*>(message_loop_)->Release();
            factory_->Release();
            factory_.release();
        }
        message_loop_ = NULL;
