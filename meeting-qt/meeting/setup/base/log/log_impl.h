...
public:
    ...
    const char* Version() const { return (const char*)"NLOGv1.0"; }
    ...
    inline void SetLevel(uint32_t level) { level_ = level; }
    ...
    void Log(uint32_t level, const char* file, uint32_t line, const char* format, ...);
...
private:
    ...
    void CreateLineSuffix(const char* file, int32_t line, int32_t level, std::string& header);
