...
class EventTrackStatisticTimer : public QTimer {
    Q_OBJECT
public:
    EventTrackStatisticTimer()
        : QTimer(nullptr)
        , time_out_cb_(nullptr) {
...
public:
    static void CreateEventTrack(const QString& action_name, const QString& module = "meeting", const QString& key = "", const QString& value = "") {
        ...
        if (!key.isEmpty() && !value.isEmpty()) {
            action->AddData(NEMeeting::utils::QStringToStdStringUTF8(key), NEMeeting::utils::QStringToStdStringUTF8(value));
        }
