...
TcpClient::TcpClient(struct event_base* event_base)
    ...
    if (sock_ == INVALID_SOCKET) {
        DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, "nbase::TcpClient: socket() error");
    } else {
...
void TcpClient::OnEvent(int fd, short event) {
    assert(fd == sock_);
    if (!connected_) {  // if not connected yet, this event means connected
        if (event & EV_WRITE) {
            // connected
            connected_ = true;
            // event_del(&event_write_);
...
int TcpClient::Connect(const char* host, int port) {
    ...
    if (r != 0 && !would_block()) {
        DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, "nbase::TcpClient: connect() error");
    }
    return r;
}
...
int TcpClient::Write(const void* data, size_t size) {
    ...
    if (n == -1 && !would_block()) {
        DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, "nbase::TcpClient: Write() send() error");
        return 0;
    }
    if (n == -1)
        n = 0;
...
int TcpClient::Read(void* buf, size_t size) {
    ...
    if (n == 0 || (n == -1 && !would_block())) {
        DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, "nbase::TcpClient: Read() recv() error");
        return 0;
    }
    if (n == -1)
        n = 0;
...
TcpConnection::TcpConnection(SOCKET fd, struct event_base* event_base)
    ...
    , listener_(0) {
    if (sock_ == INVALID_SOCKET) {
        DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, "nbase::TcpConnection: invalid socket");
    } else {
        set_block(sock_, false);
...
int TcpConnection::Write(const void* data, size_t size) {
    int n = ::send(sock_, (const char*)data, size, 0);
    if (n == -1 && !would_block()) {
        DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, "nbase::TcpConnection: Write() send() error");
        return 0;
    }
    if (n == -1)
        n = 0;
...
int TcpConnection::Read(void* buf, size_t size) {
    int n = ::recv(sock_, (char*)buf, size, 0);
    if (n == 0 || (n == -1 && !would_block())) {
        DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, "nbase::TcpConnection: Read() recv() error");
        return 0;
    }
    if (n == -1)
        n = 0;
...
TcpServer::TcpServer(struct event_base* event_base, uint16_t port, const char* host)
    ...
    sock_ = ::socket(AF_INET, SOCK_STREAM, 0);
    if (sock_ == INVALID_SOCKET) {
        DEFLOG(nbase::LogInterface::LV_ERR, __FILE__, __LINE__, "nbase::TcpServer: invalid socket");
    } else {
        set_block(sock_, false);
        reuse_address(sock_);
