...
void AuthManager::onLoginCallback(int code, const QJsonObject& response, bool registerMode) {
    if (0 == code) {
        ...
        setPersonalShortMeetingNum(response["shortMeetingNum"].toString());
        // setAPaasAppKey(response["appKey"].toString());
...
QString AuthManager::personalShortMeetingNum() const {
    return m_shortPersonalMeetingId;
}

void AuthManager::setPersonalShortMeetingNum(const QString& shortMeetingNum) {
