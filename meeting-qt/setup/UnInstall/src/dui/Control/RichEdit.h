...
public:
    ...
    VerAlignType GetTextVerAlignType() { return m_textVerAlignType; }

    void AttachReturn(const EventCallback& callback) { OnEvent[EventType::RETURN] += callback; }

    void AttachTab(const EventCallback& callback) { OnEvent[EventType::TAB] += callback; }

    void AttachTextChange(const EventCallback& callback) { OnEvent[EventType::TEXTCHANGE] += callback; }
...
protected:
    ...
    bool m_bSelAllEver = false;       //ֻڵһʱȫѡ
    bool m_bNoSelOnKillFocus = true;  // m_bEnabled && m_bReadOnly
    bool m_bSelAllOnFocus = false;    // m_bEnabled && !m_bReadOnly
