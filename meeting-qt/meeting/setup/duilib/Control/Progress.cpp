...
void Progress::SetProgressColor(const std::wstring& dwProgressColor) {
    ...
    if (m_dwProgressColor == dwProgressColor)
        return;
...
void Progress::SetAttribute(const std::wstring& pstrName, const std::wstring& pstrValue) {
    ...
    else if (pstrName == _T("min"))
        SetMinValue(_ttoi(pstrValue.c_str()));
    else if (pstrName == _T("max"))
        SetMaxValue(_ttoi(pstrValue.c_str()));
    else if (pstrName == _T("value"))
        SetValue(_ttoi(pstrValue.c_str()));
    else if (pstrName == _T("isstretchfore"))
        SetStretchForeImage(pstrValue == _T("true"));
    ...
    } else if (pstrName == _T("progressimage"))
        SetProgressImage(pstrValue);
    else
...
void Progress::PaintStatusImage(HDC hDC) {
    ...
    if (!m_progressImage.imageAttribute.imageString.empty()) {
        ...
        if (m_bStretchForeImage)
            ...
            m_progressImageModify = StringHelper::Printf(_T("dest='%d,%d,%d,%d' source='%d,%d,%d,%d'"), rc.left, rc.top, rc.right, rc.bottom, rc.left,
