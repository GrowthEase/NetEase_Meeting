...
template <typename CharType>
size_t StringReplaceAllT(const std::basic_string<CharType>& find, const std::basic_string<CharType>& replace, std::basic_string<CharType>& output) {
    ...
    if (find_length < replace_length) {
        ...
        memmove((void*)(data_ptr + offset), (void*)data_ptr, (output.size() - offset) * sizeof(CharType));
    } else {
        endpos = output.size();
        offset = 0;
    ...
    while (offset < endpos) {
        ...
        if (found_pos != std::basic_string<CharType>::npos) {
            ...
            if (target != found_pos)
                memmove((void*)(data_ptr + target), (void*)(data_ptr + offset), (found_pos - offset) * sizeof(CharType));
            ...
            /* ending work  */
            if (target != offset)
                memcpy((void*)(data_ptr + target), (void*)(data_ptr + offset), (endpos - offset) * sizeof(CharType));
            break;
        }
    }
...
void UTF8ToUTF16(const UTF8Char* utf8, size_t length, std::basic_string<UTF16Char>& utf16) {
    ...
    while (src_begin < src_end) {
        ConversionResult result = ConvertUTF8toUTF16(&src_begin, src_end, &dst_begin, dst_begin + COUNT_OF(output), lenientConversion);
...
void UTF16ToUTF8(const UTF16Char* utf16, size_t length, std::basic_string<UTF8Char>& utf8) {
    ...
    utf8.clear();
    while (src_begin < src_end) {
        ConversionResult result = ConvertUTF16toUTF8(&src_begin, src_end, &dst_begin, dst_begin + COUNT_OF(output), lenientConversion);
...
void UTF8ToUTF32(const UTF8Char* utf8, size_t length, std::basic_string<UTF32Char>& utf32) {
    ...
    utf32.clear();
    while (src_begin < src_end) {
        ConversionResult result = ConvertUTF8toUTF32(&src_begin, src_end, &dst_begin, dst_begin + COUNT_OF(output), lenientConversion);
...
void UTF32ToUTF8(const UTF32Char* utf32, size_t length, std::basic_string<UTF8Char>& utf8) {
    UTF8Char output[8192];
    ...
    utf8.clear();
    while (src_begin < src_end) {
        ConversionResult result = ConvertUTF32toUTF8(&src_begin, src_end, &dst_begin, dst_begin + COUNT_OF(output), lenientConversion);
...
void UTF16ToUTF32(const UTF16Char* utf16, size_t length, std::basic_string<UTF32Char>& utf32) {
    UTF32Char output[4096];
    const UTF16* src_begin = reinterpret_cast<const UTF16*>(utf16);
    const UTF16* src_end = src_begin + length;
    UTF32* dst_begin = reinterpret_cast<UTF32*>(output);
    ...
    utf32.clear();
    while (src_begin < src_end) {
        ConversionResult result = ConvertUTF16toUTF32(&src_begin, src_end, &dst_begin, dst_begin + COUNT_OF(output), lenientConversion);
...
void UTF32ToUTF16(const UTF32Char* utf32, size_t length, std::basic_string<UTF16Char>& utf16) {
    ...
    utf16.clear();
    while (src_begin < src_end) {
        ConversionResult result = ConvertUTF32toUTF16(&src_begin, src_end, &dst_begin, dst_begin + COUNT_OF(output), lenientConversion);
...
void UTF8ToUTF32(const std::basic_string<UTF8Char>& utf8, std::basic_string<UTF32Char>& utf32) {
    UTF8ToUTF32(utf8.c_str(), utf8.length(), utf32);
}

void UTF32ToUTF8(const std::basic_string<UTF32Char>& utf32, std::basic_string<UTF8Char>& utf8) {
    UTF32ToUTF8(utf32.c_str(), utf32.length(), utf8);
}

void UTF16ToUTF32(const std::basic_string<UTF16Char>& utf16, std::basic_string<UTF32Char>& utf32) {
    UTF16ToUTF32(utf16.c_str(), utf16.length(), utf32);
}

void UTF32ToUTF16(const std::basic_string<UTF32Char>& utf32, std::basic_string<UTF16Char>& utf16) {
    UTF32ToUTF16(utf32.c_str(), utf32.length(), utf16);
}
