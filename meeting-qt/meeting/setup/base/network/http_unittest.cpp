...
TEST(HttpTest, HttpPost) {
    ...
    nbase::HttpError error = http.HttpPost("http://reg.163.com/services/userlogin",
                                           "username=po_mail_test&password=d078f151fdcf4415082b68887d3cb1d0&type=1&product=popomail", content);
    EXPECT_EQ(CURLE_OK, error);
    EXPECT_TRUE(content.size() > 0);
