...
public:
    ...
    void Start() { timer_.Start(nbase::TimeDelta::FromMilliseconds(delay_ms_), this, &OneShotTimerTester::Run); }
...
public:
    ...
    void Start() { timer_->Start(nbase::TimeDelta::FromMilliseconds(10), this, &OneShotSelfDeletingTimerTester::Run); }
...
public:
    ...
    void Start() { timer_.Start(nbase::TimeDelta::FromMilliseconds(10), this, &RepeatingTimerTester::Run); }
...
TEST(TimerTest, OneShotTimer) {
    ...
    a.message_loop()->PostNonNestableTask(nbase::NewRunnableFunction(&RunTest_OneShotTimer, &f));
...
TEST(TimerTest, OneShotSelfDeletingTimer) {
    ...
    a.message_loop()->PostNonNestableTask(nbase::NewRunnableFunction(&RunTest_OneShotSelfDeletingTimer, &f));
    for (int i = 100; i >= 0 && !did_run; --i) {
        nbase::Thread::Sleep(10);
    }
...
TEST(TimerTest, RepeatingTimer) {
    ...
    a.message_loop()->PostNonNestableTask(nbase::NewRunnableFunction(&RunTest_RepeatingTimer, &f));
    for (int i = 100; i >= 0 && !did_run; --i) {
        nbase::Thread::Sleep(10);
    }
