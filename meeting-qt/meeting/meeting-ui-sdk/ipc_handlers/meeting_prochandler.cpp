...
bool NEMeetingServiceProcHandlerIMP::onStartMeeting(const NS_I_NEM_SDK::NEStartMeetingParams& param,
    ...
    YXLOG_API(Info) << "Received start meeting request, meeting ID: " << param.meetingNum << ", nickname: " << param.displayName
    ...
    if (!param.meetingNum.empty() && param.meetingNum != AuthManager::getInstance()->getAuthInfo().personalRoomId) {
    ...
    Invoker::getInstance()->execute([=]() {
        ...
        SettingsManager::getInstance()->setDetectMutedMic(opts.detectMutedMic);
        SettingsManager::getInstance()->setUnpubAudioOnMute(opts.unpubAudioOnMute);
        if (opts.full_more_menu_items_.size() > 0) {
...
bool NEMeetingServiceProcHandlerIMP::onGetCurrentMeetingInfo(const nem_sdk_interface::NEMeetingService::NEGetMeetingInfoCallback& cb) {
    ...
    Invoker::getInstance()->execute([=]() {
        ...
        info.meetingNum = meetingInfo.roomId;
        info.shortMeetingNum = meetingInfo.shortRoomId;
        info.duration = MeetingManager::getInstance()->meetingDuration() * 1000;
        info.hostUserId = meetingInfo.hostAccountId;
        info.meetingId = std::atoll(meetingInfo.roomUniqueId.c_str());
        ...
        YXLOG(Info) << "Got current meeting info, meeting ID: " << info.meetingNum << ", shortId: " << info.shortMeetingNum
...
void NEMeetingServiceProcHandlerIMP::onGetPresetMenuItems(const std::vector<int>& menuItemsId,
    ...
    Invoker::getInstance()->execute([=]() {
        ...
        YXLOG(Info) << "PresetMenuItems size: " << items.size() << YXLOGEnd;
...
void NEMeetingServiceProcHandlerIMP::onMeetingStatusChanged(NEMeeting::Status status, int errorCode, const QString& errorMessage) {
    ...
    if (NEMeeting::MEETING_CONNECT_FAILED == status &&
        ...
        QString errorMsg;
        if ("joinTimeoutTimer" == errorMessage)
            errorMsg = tr("joinTimeoutTimer");
        else if ("kReasonRoomNotExist" == errorMessage)
            errorMsg = tr("kReasonRoomNotExist");
        else if ("kReasonSyncDataError" == errorMessage)
            errorMsg = tr("kReasonSyncDataError");
        else if ("kReasonRtcInitError" == errorMessage)
            errorMsg = tr("kReasonRtcInitError");
        else if ("kReasonJoinChannelError" == errorMessage)
            errorMsg = tr("kReasonJoinChannelError");

        if (m_startMeetingCallback != nullptr) {
            YXLOG(Info) << "Invoke start meeting callback, errorCode: " << errorCode << ", errorMessage: " << errorMsg.toStdString() << YXLOGEnd;
            m_startMeetingCallback(responseCode, errorMsg.toStdString());
            m_startMeetingCallback = nullptr;
        }

        if (m_joinMeetingCallback != nullptr) {
            YXLOG(Info) << "Invoke join meeting callback, errorCode: " << errorCode << ", errorMessage: " << errorMsg.toStdString() << YXLOGEnd;
            m_joinMeetingCallback(responseCode, errorMsg.toStdString());
        ...
        for (auto item = items.begin(); item != items.end(); item++) {
            ...
            do {
                ...
                if (!bInjected && ((!strItemTitle2.isEmpty() && strItemImage2.isEmpty()) || (!strItemImage2.isEmpty() && strItemTitle2.isEmpty()))) {
                    YXLOG(Warn) << "checkOptions." << YXLOGEnd;
                    break;
                }
                ...
                if (converted.length() > MENU_ITEM_TITLE_LIMIT) {
                    YXLOG(Warn) << "checkOptions, title: " << item->itemTitle << ", titleLength: " << item->itemTitle.length()
                                << ", length: " << converted.length() << YXLOGEnd;
                    break;
                }
                ...
                if (!bInjected && !strItemTitle2.isEmpty()) {
                    ...
                    if (converted.length() > MENU_ITEM_TITLE_LIMIT) {
                        YXLOG(Warn) << "checkOptions, title2: " << item->itemTitle2 << ", titleLength: " << item->itemTitle2.length()
                                    << ", length: " << converted.length() << YXLOGEnd;
                        break;
                    }
                }
