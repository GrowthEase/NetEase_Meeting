...
std::wstring CSetupData::GetDefaultInstallDir() {
    GetLastInstDir();
    std::wstring fullFolderPath = GetFullFolderPath($PROGRAMFILES PRODUCT_DOC);
    std::wstring strPreInstPathTmp = m_strPreInstPath;
    std::wstring fullFolderPathTmp = fullFolderPath;
    std::transform(strPreInstPathTmp.begin(), strPreInstPathTmp.end(), strPreInstPathTmp.begin(), towlower);
    std::transform(fullFolderPathTmp.begin(), fullFolderPathTmp.end(), fullFolderPathTmp.begin(), towlower);

    std::wstring strTmp = L" (x86)";
#ifdef PRODUCT_EXE_X64
    auto nPos = fullFolderPath.find(strTmp);
    if (std::wstring::npos != nPos) {
        fullFolderPath.replace(nPos, strTmp.size(), L"");
    }
#else
    auto nPos = fullFolderPathTmp.find(strTmp);
    if (std::wstring::npos != nPos) {
        fullFolderPathTmp.replace(nPos, strTmp.size(), L"");
    }
#endif  // PRODUCT_EXE_X64

    if (fullFolderPathTmp == strPreInstPathTmp) {
        return fullFolderPath;
    } else {
        return m_strPreInstPath.empty() ? fullFolderPath : m_strPreInstPath;
    }
}
...
std::wstring CSetupData::GetOldSubProcessName() {
    std::wstring process_name = OLD_SUB_PROCESS_NAME;
    return process_name;
}

std::wstring CSetupData::GetSubProcessName() {
    std::wstring process_name = SUB_PROCESS_NAME;
    return process_name;
}
...
int CSetupData::GetDelFileInfoListNum() {
    if (m_strAryDleFileList.size() == 0) {
        ...
        //删除测试配置
        DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localAppKey");
        DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localServerAddress");
        DeleteRegValue(HKEY_CURRENT_USER, L"Software\\Netease\\Meeting", L"localPaasServerAddress");
    }
...
int CSetupData::GetAddFileInfoListNum() {
    if (m_vecCopyFileInfoList.size() == 0) {
        _AddFile_copy(L"release\\", $INSTDIR);
    }
...
int CSetupData::GetAddRegInfoListNum() {
    if (m_vecAddRegInfoList.size() == 0) {
        ...
        _WriteRegValue(HKCR, START_CMD_KEY, L"", PRODUCT_NAME);
        _WriteRegValue(HKCR, START_CMD_KEY, L"URL Protocol", $INSTDIR RUN_NAME);
        _WriteRegValue(HKCR, START_CMD_KEY L"\\shell\\open\\command", L"", L"\"" $INSTDIR RUN_NAME L"\" --url=\"%1\"");
        _WriteRegValue(HKLM, L"SOFTWARE\\Classes\\" START_CMD_KEY L"\\shell\\open\\command", L"", L"\"" $INSTDIR RUN_NAME L"\" --url=\"%1\"");
    }
...
void CSetupData::StartRun(std::wstring cmd) {
    ...
    if (GetVersionEx((OSVERSIONINFO*)&osver)) {
        if (VER_PLATFORM_WIN32_NT == osver.dwPlatformId && osver.dwMajorVersion > 5) {
            if (/*CreateLowIntegrityProcess(str.c_str(), cmd.c_str())*/ CreateProcessLow((wchar_t*)str.c_str(), (wchar_t*)cmd.c_str(), NULL,
                                                                                         SW_SHOW)) {
                LOG_APP(L"start_run CreateLowIntegrityProcess %s, %s", str.c_str(), cmd.c_str());
                return;
            }
        }
    }
    ...
    int ret = (int)inst;
    if (ret > 32) {
        LOG_APP(L"start_run %s, %s", str.c_str(), cmd.c_str());
    } else {
        LOG_ERR(L"start_run %s, %s failed %d", str.c_str(), cmd.c_str(), ret);
    }
}
...
bool CSetupData::FindAppProcessID(const std::wstring& strExeFile, std::vector<int>& exe) {
    ...
    if (::Process32First(handle, &process)) {
        do {
            if (strExeFile == std::wstring(process.szExeFile)) {
                HANDLE moduleHandle = ::CreateToolhelp32Snapshot(TH32CS_SNAPMODULE, process.th32ProcessID);
                MODULEENTRY32 module = {0};
                module.dwSize = sizeof(MODULEENTRY32);
                if (::Module32First(moduleHandle, &module)) {
                    std::wstring modulePath = module.szExePath;
                    std::wstring exeFlag = PRODUCT_DOC;
                    exeFlag += module.szModule;
                    std::transform(modulePath.begin(), modulePath.end(), modulePath.begin(), towlower);
                    std::transform(exeFlag.begin(), exeFlag.end(), exeFlag.begin(), towlower);
                    if (modulePath.find(exeFlag) != std::string::npos) {
                        find = true;
                        exe.push_back(process.th32ProcessID);
                    }
                }
...
void CSetupData::CloseAppProcess() {
    ...
    exe.clear();
    if (FindAppProcessID(SUB_PROCESS_NAME, exe)) {
        for (auto& i : exe) {
            HANDLE ProcessHandle = OpenProcess(PROCESS_ALL_ACCESS, FALSE, i);
            if (ProcessHandle) {
                ::TerminateProcess(ProcessHandle, 0);
    ...
    exe.clear();
    if (FindAppProcessID(OLD_SUB_PROCESS_NAME, exe)) {
        for (auto& i : exe) {
            HANDLE ProcessHandle = OpenProcess(PROCESS_ALL_ACCESS, FALSE, i);
            if (ProcessHandle) {
                ::TerminateProcess(ProcessHandle, 0);
