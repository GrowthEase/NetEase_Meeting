...
export default function useInterpreterModal(data: {
  ...
  useEffect(() => {
    if (isHostOrCoHost) {
      eventEmitter?.on(EventType.OnInterpreterLeave, () => {
        ...
        interpreterLeaveRef.current = Modal.confirm({
          title: t('commonTitle'),
          width: 270,
  ...
  useEffect(() => {
    ...
    eventEmitter?.on(EventType.MyInterpreterRemoved, () => {
      ...
      showMyInterpreterRemovedRef.current = Modal.confirm({
        width: 270,
        title: t('commonTitle'),
        okText: t('globalView'),
  ...
  }, [eventEmitter, t, defaultListeningVolume, globalDispatch, neMeeting])
  ...
  }, [meetingInfo.isInterpreter, eventEmitter, meetingInfo.interpretation])
