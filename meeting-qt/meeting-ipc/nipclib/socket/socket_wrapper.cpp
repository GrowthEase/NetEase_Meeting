...
#define WOULD_BLOCK(status) \
    ((status) == TNET_ERROR_WOULDBLOCK || (status) == TNET_ERROR_ISCONN || (status) == TNET_ERROR_INTR || (status) == TNET_ERROR_INPROGRESS)
#else
#define WOULD_BLOCK(status) \
    ((status) == TNET_ERROR_WOULDBLOCK || (status) == TNET_ERROR_ISCONN || (status) == TNET_ERROR_INPROGRESS || (status) == TNET_ERROR_EAGAIN)
#endif

bool would_block(bool connecting) {
    int err = tnet_geterrno();
#if defined(DEBUG) || defined(_DEBUG)
    if (err == TNET_ERROR_WOULDBLOCK || err == TNET_ERROR_INPROGRESS ||
        ((err == 0) && (true == connecting)) || /*解决browser manager程序管理软件导致socket连接建立失败的问题。modified by HarrisonFeng, 2012-10-11*/
        err == 2) /*Because I reset socket receive buffer size in my program. And in debug mode, system will not alloc system buffer for the socket if
                     no data arrive so it may return value 2 means file not ready or exist. If the data already arrive, even one and only one byte,
                     system will alloc the buffer size as I reset, and then it will go well. In release mode, buffer will immediately alloc after
                     reset buffer size of socket, so it will go well. added by HarrisonFeng, 2011-08-10*/
#else
    if (err == TNET_ERROR_WOULDBLOCK || err == TNET_ERROR_INPROGRESS ||
        ((err == 0) && (true == connecting))) /*解决browser manager程序管理软件导致socket连接建立失败的问题。modified by HarrisonFeng, 2012-10-11*/
#endif
...
bool TcpClientImpl::Init(const std::string& host, int port) {
    ...
    if (((fd_ = tnet_transport_connectto_2(socket_handle_, host.c_str(), port)) == TNET_INVALID_FD) && !would_block(true))  //连接过程中判断是否阻塞
    {
        return false;
    }
    tnet_ip_t ip_t;
...
int TcpClientImpl::Write(const void* data, size_t size) {
    auto tcp_client = shared_from_this();
    if (nullptr == socket_handle_) {
        if (tcp_client) {
            std::string str = "socket_handle_ is null.";
            tcp_client->OnError(SOCKET_ERROR, str.c_str(), str.size());
        }
        // TODO(litianyi) 原因 20160427，最近可以追叙的bug是2.1.0版本 NIM-3713
        // PC端网络断开时会有一定几率导致崩溃，时间为3月28日，需要追查之前的代码改动
        return SOCKET_ERROR;
    }
    int n = tnet_transport_send(socket_handle_, fd_, data, (uint32_t)size);  //发送失败时返回0
    if (0 == n) {
        if (tcp_client) {
            std::string str = "tnet_transport_send failed, ret: 0";
            tcp_client->OnError(SOCKET_ERROR, str.c_str(), str.size());
        }
        //如果是阻塞，则不返回SOCKET_ERROR错误，否则返回SOCKET_ERROR错误！modified by HarrisonFeng, 2013.11.1
        if (!would_block()) {
            if (tcp_client) {
                std::string str = "would_block is not block.";
                tcp_client->OnError(SOCKET_ERROR, str.c_str(), str.size());
            }
        ...
        if (tcp_client) {
            std::string str = "tnet_transport_send successfully.";
            tcp_client->OnError(SOCKET_ERROR, str.c_str(), str.size());
        }
    }
    return n;
...
void TcpClientImpl::OnError(int error_code, const void* data, size_t size) {
    if (handler_)
