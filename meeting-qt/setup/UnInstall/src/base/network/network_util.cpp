...
#include <nb30.h>
#include <snmp.h>
//#include "third_party/boost/regex.hpp"
#else
#include <net/if_dl.h>
...
bool GetMacAddressByNetBIOS(std::string& mac_address) {
    ...
    for (i = 0; i < lenum.length; ++i) {
        ...
        uRetCode = Netbios(&Ncb);
        if (uRetCode == 0) {
            if (Adapter.adapt.adapter_address[0] + Adapter.adapt.adapter_address[1] + Adapter.adapt.adapter_address[2] +
                    Adapter.adapt.adapter_address[3] + Adapter.adapt.adapter_address[4] + Adapter.adapt.adapter_address[5] !=
                0) {
                StringPrintf(mac_address, "%02x-%02x-%02x-%02x-%02x-%02x", Adapter.adapt.adapter_address[0], Adapter.adapt.adapter_address[1],
...
bool GetMacAddressBySNMP(std::string& mac_address) {
    ...
    for (int j = 0; j < var_bind[0].value.asnValue.number; j++) {
        ...
        if ((var_bind[1].value.asnValue.address.stream[0] == 0x44) && (var_bind[1].value.asnValue.address.stream[1] == 0x45) &&
            (var_bind[1].value.asnValue.address.stream[2] == 0x53) && (var_bind[1].value.asnValue.address.stream[3] == 0x54) &&
            (var_bind[1].value.asnValue.address.stream[4] == 0x00))
            continue;
        // Ignore NULL addresses returned by other network interfaces
        if ((var_bind[1].value.asnValue.address.stream[0] == 0x00) && (var_bind[1].value.asnValue.address.stream[1] == 0x00) &&
            (var_bind[1].value.asnValue.address.stream[2] == 0x00) && (var_bind[1].value.asnValue.address.stream[3] == 0x00) &&
            (var_bind[1].value.asnValue.address.stream[4] == 0x00) && (var_bind[1].value.asnValue.address.stream[5] == 0x00))
            continue;

        StringPrintf(mac_address, "%02x-%02x-%02x-%02x-%02x-%02x", var_bind[1].value.asnValue.address.stream[0],
                     var_bind[1].value.asnValue.address.stream[1], var_bind[1].value.asnValue.address.stream[2],
                     var_bind[1].value.asnValue.address.stream[3], var_bind[1].value.asnValue.address.stream[4],
...
bool GetMacAddressByAdaptersInfo(std::string& mac_address) {
    ...
    if (GetAdaptersInfo(adapter_info, &out_buf_len) == NO_ERROR) {
        ...
        for (; adapter != NULL; adapter = adapter->Next) {
            ...
            StringPrintf(mac_address, "%02x-%02x-%02x-%02x-%02x-%02x", int(adapter->Address[0]), int(adapter->Address[1]), int(adapter->Address[2]),
                         int(adapter->Address[3]), int(adapter->Address[4]), int(adapter->Address[5]));
...
bool GetMacAddressByAdaptersAddresses(std::string& mac_address) {
    ...
    if (GetAdaptersAddresses(AF_UNSPEC, 0, NULL, addresses, &out_buf_len) == NO_ERROR) {
        ...
        for (; curr_addresses != NULL; curr_addresses = curr_addresses->Next) {
            ...
            StringPrintf(mac_address, "%02x-%02x-%02x-%02x-%02x-%02x", int(curr_addresses->PhysicalAddress[0]),
                         int(curr_addresses->PhysicalAddress[1]), int(curr_addresses->PhysicalAddress[2]), int(curr_addresses->PhysicalAddress[3]),
                         int(curr_addresses->PhysicalAddress[4]), int(curr_addresses->PhysicalAddress[5]));
...
bool IsInternalIP(const uint32_t ip) {
    if ((ip >= 0x0A000000 && ip <= 0x0AFFFFFF) || (ip >= 0xAC100000 && ip <= 0xAC1FFFFF) || (ip >= 0xC0A80000 && ip <= 0xC0A8FFFF))
        return true;
    else
        return false;
}
