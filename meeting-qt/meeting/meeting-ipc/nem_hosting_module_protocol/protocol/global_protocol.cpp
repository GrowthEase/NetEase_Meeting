...
void InitRequest::OnPack(Json::Value& root) const {
    ...
    root["language"] = (int)init_config_.getLanguage();
    root["serverUrl"] = init_config_.getServerUrl();
...
void InitRequest::OnParse(const Json::Value& root) {
    ...
    init_config_.setLanguage((NEMeetingLanguage)root["language"].asInt());
    init_config_.setServerUrl(root["serverUrl"].asString());
