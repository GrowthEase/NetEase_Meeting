...
bool CreateDirectory(const PathChar* full_path) {
    ...
    for (auto i = subpaths.begin(); i != subpaths.end(); ++i) {
        ...
        if (!::CreateDirectoryW(i->c_str(), NULL)) {
            ...
            if (error == ERROR_ALREADY_EXISTS && FilePathIsExist(i->c_str(), true)) {
                continue;
            }
            return false;
        }
...
int ReadFile(const PathChar* filepath, void* data_out, size_t size) {
    win32::ScopedWinHandle file(
        CreateFileW(filepath, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL));
    if (!file)
        return -1;

    DWORD read;
    if (::ReadFile(file, data_out, size, &read, NULL) && static_cast<int>(read) == size)
        return read;
    return -1;
}

int WriteFile(const PathChar* filepath, const void* data, size_t size) {
    win32::ScopedWinHandle file(CreateFileW(filepath, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 0, NULL));
    if (!file)
        return -1;
...
bool CopyFile(const PathString& from_path, const PathString& to_path) {
    if (from_path.size() >= MAX_PATH || to_path.size() >= MAX_PATH) {
        return false;
    }
    return (::CopyFileW(from_path.c_str(), to_path.c_str(), false) != 0);
}
