...
void LibuvMessagePump::OnLibuvNotification(uv_poll_t* req, int status, int events) {
...
bool LibuvMessagePump::Watcher::StartPoll(LibuvMessagePump* pump, Event mode) {
    ...
    if (handle_ == NULL) {
        ...
        int r = is_socket_ ? uv_poll_init_socket(pump->loop_, handle_, fd_) : uv_poll_init(pump->loop_, handle_, fd_);
        if (r) {
...
void LibuvMessagePump::Watcher::OnUVClose(uv_handle_t* handle) {
    LibuvMessagePump::Watcher* self = reinterpret_cast<LibuvMessagePump::Watcher*>(handle->data);
