...
void NEFeedbackServiceIMP::feedback(const int& type, const std::string& path, bool needAudioDump, const NEFeedbackCallback& cb) {
    PostTaskToProcThread(ToWeakCallback([this, type, path, needAudioDump, cb]() {
        FeedbackRequest request;
        request.path_ = path;
        request.type_ = type;
        request.needAudioDump_ = needAudioDump;
