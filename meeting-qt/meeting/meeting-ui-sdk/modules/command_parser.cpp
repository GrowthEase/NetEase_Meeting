...
CommandParser::CommandParser(QObject* parent)
    : QObject(parent)
    , m_organizationName(QStringList() << "o"
                                       << "organization",
                         "Set your organization name.",
                         "organization")
    , m_applicationName(QStringList() << "n"
                                      << "applicationName",
                        "Set your application name.",
                        "applicationName")
    , m_applicationDomain(QStringList() << "d"
                                        << "domain",
                          "Set your organization domain.",
                          "domain")
    , m_applicationDisplayName(QStringList() << "s"
                                             << "displayName",
                               "Set your application display name.",
                               "displayName")
    , m_createMeeting(QStringList() << "c"
                                    << "create",
                      "Create a meeting with this ID.",
                      "create")
    , m_joinMeeting(QStringList() << "j"
                                  << "join",
                    "Join a meeting with this ID.",
                    "join")
    , m_accountId(QStringList() << "i"
                                << "accountId",
                  "Set your account ID.",
                  "accountId")
    , m_accountToken(QStringList() << "t"
                                   << "accountToken",
                     "Set your account token.",
                     "accountToken")
    , m_nickname(QStringList() << "m"
                               << "nickname",
                 "Set your nickname of meeting.",
                 "nickname")
    , m_appKey(QStringList() << "k"
                             << "appKey",
               "Set your application key.",
               "appKey")
    , m_enableAudio(QStringList() << "a"
                                  << "enableAudio",
                    "Set your application key.",
                    "enableAudio")
    , m_enableVideo(QStringList() << "v"
                                  << "enableVideo",
                    "Set your application key.",
                    "enableVideo")
    , m_hideInvitButton(QStringList() << "h"
                                      << "hideInvitation",
                        "Hidden invitation button in meeting.",
                        "hideInvitation")
    , m_logoFile(QStringList() << "l"
                               << "logoFile",
                 "Specify the logo path.",
                 "logoFile")
    , m_ipcPort(QStringList() << "p"
                              << "port",
                "Application will run as this port.",
                "port")
    , m_multClient(QStringList() << "e"
                                 << "multClient",
                   "Application supports multiple open.",
                   "multClient")
    , m_ipcClientPort(0) {
    m_argumentsParser.addOptions({m_organizationName, m_applicationName, m_applicationDomain, m_applicationDisplayName, m_createMeeting,
                                  m_joinMeeting, m_accountId, m_accountToken, m_nickname, m_appKey, m_enableAudio, m_enableVideo, m_hideInvitButton,
                                  m_logoFile, m_ipcPort, m_multClient});
}
...
RunType CommandParser::parseCommandLine(const QGuiApplication& app) {
    ...
    } else if (m_argumentsParser.isSet(m_accountId) && m_argumentsParser.isSet(m_accountToken) && m_argumentsParser.isSet(m_appKey)) {
