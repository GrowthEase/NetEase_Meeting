...
static inline void DrawFunction(HDC hdcDest, bool bCurCanvasTransparent, UiRect rcDest, HDC hdcSrc, UiRect rcSrc, bool alphaChannel, int uFade) {
    ...
    ::AlphaBlend(hdcDest, rcDest.left, rcDest.top, rcDest.GetWidth(), rcDest.GetHeight(), hdcSrc, rcSrc.left, rcSrc.top, rcSrc.GetWidth(),
                 rcSrc.GetHeight(), ftn);
    //}
    // else {
...
void RenderEngine::GdiDrawImage(HDC hDC,
                                bool bCurCanvasTransparent,
                                const UiRect& rcPaint,
                                HBITMAP hBitmap,
                                bool alphaChannel,
                                const UiRect& rcImageDest,
                                const UiRect& rcImageSource,
                                const UiRect& rcCorners,
                                BYTE uFade,
                                bool xtiled,
                                bool ytiled) {
    UiRect rcTestTemp;
...
void RenderEngine::DrawText(HDC hDC,
                            UiRect& rc,
...
UiRect RenderEngine::MeasureText(HDC hDC, const std::wstring& strText, int iFont, UINT uStyle, int width) {
    ...
    if (width == DUI_NOSET_VALUE) {
        ...
        if ((uStyle & DT_SINGLELINE) != 0) {
            ...
            graphics.MeasureString(L"", 2, &font, rectF, &stringFormat, &bounds);
