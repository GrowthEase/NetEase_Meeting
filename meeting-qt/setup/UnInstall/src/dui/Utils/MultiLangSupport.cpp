...
bool MutiLanSupport::LoadLangSetting() {
    ...
    if (lang_setting_.lang_name.empty()) {
        lang_setting_.lang_name = kDefaultLangName;  //Ĭ
    }
    return true;
}

void MutiLanSupport::SetLangSetting(const LangSetting& new_lang_setting) {
    if (new_lang_setting_.lang_name == new_lang_setting.lang_name) {  //Ҫ±棬µϢ
...
BOOL MutiLanSupport::SwitchLang(const std::wstring& new_lang) {
    ...
    if (SaveLangSetting()) {
        ...
        if (!MutiLanSupport::GetInstance()->LoadStringTable(current_app_folder + kLangFolderName + L"\\" + new_lang + L"\\gdstrings.ini")) {
            return FALSE;
        }
        return TRUE;
    }
