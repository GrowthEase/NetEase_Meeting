...
bool Facade::SetAttribute(const std::wstring& pstrName, const std::wstring& pstrValue) {
    ...
    if (pstrName == _T("itemfont"))
        nFont = _ttoi(pstrValue.c_str());
    ...
    } else if (pstrName == _T("itemnoramlimage"))
        SetItemStateImage(ControlStateType::NORMAL, pstrValue);
    else if (pstrName == _T("itemaltbk"))
        SetAlternateBk(pstrValue == _T("true"));
    else if (pstrName == _T("itemselectedtextcolor")) {
        DWORD clrColor = GlobalManager::ConvertTextColor(pstrValue);
        SetSelectedItemTextColor(clrColor);
    } else if (pstrName == _T("itemselectedbkcolor")) {
        SetSelectedItemBkColor(pstrValue);
    } else if (pstrName == _T("itemselectedimage"))
        SetItemStateImage(ControlStateType::PUSHED, pstrValue);
    else if (pstrName == _T("itemhottextcolor")) {
        DWORD clrColor = GlobalManager::ConvertTextColor(pstrValue);
        SetHotItemTextColor(clrColor);
    } else if (pstrName == _T("itemhotbkcolor")) {
        SetItemBkColor(ControlStateType::HOT, pstrValue);
    } else if (pstrName == _T("itemhotimage"))
        SetItemStateImage(ControlStateType::HOT, pstrValue);
    ...
    } else if (pstrName == _T("itemdisabledimage"))
        SetItemStateImage(ControlStateType::DISABLED, pstrValue);
    else {
        hasAttribute = false;
    }
...
bool ListBox::SetItemIndex(Control* pControl, std::size_t iIndex) {
    ...
    if (m_iCurSel >= 0)
        pSelectedListItem = dynamic_cast<IListItem*>(GetItemAt(m_iCurSel));
...
void ListBox::EnsureVisible(const UiRect& rcItem) {
    ...
    if (rcNewItem.left >= rcList.left && rcNewItem.top >= rcList.top && rcNewItem.right <= rcList.right && rcNewItem.bottom <= rcList.bottom) {
...
bool ListBox::AddAt(Control* pControl, int iIndex) {
    ...
    if (!ScrollableBox::AddAt(pControl, iIndex))
        return false;
...
bool ListBox::RemoveAt(int iIndex) {
    if (!ScrollableBox::RemoveAt(iIndex))
        return false;
...
bool ListBox::SelectItem(int iIndex, bool bTakeFocus, bool bTrigger) {
    ...
    if (m_iCurSel >= 0) {
        ...
        if (pControl != NULL) {
            IListItem* pListItem = dynamic_cast<IListItem*>(pControl);
            if (pListItem != NULL)
                pListItem->Select(false, bTrigger);
        }
...
void ListContainerElement::InvokeDoubleClickEvent() {
    if (m_pWindow != NULL)
        m_pWindow->SendNotify(this, EventType::DOUBLECLICK);
}
...
void ListContainerElement::HandleMessage(EventArgs& event) {
    if (!IsMouseEnabled() && event.Type > EventType::MOUSEBEGIN && event.Type < EventType::MOUSEEND) {
        ...
        if (event.chKey == VK_RETURN) {
            if (IsActivatable()) {
                if (m_pWindow != NULL)
                    m_pWindow->SendNotify(this, EventType::RETURN);
            }
            return;
        }
...
void ListLabelElement::DrawItemText(HDC hDC, const UiRect& rcItem) {
    ...
    RenderEngine::DrawText(hDC, rcText, GetText(), iTextColor, pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
}
