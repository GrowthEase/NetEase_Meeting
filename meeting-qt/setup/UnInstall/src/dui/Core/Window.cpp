    ...
    while (*pStrImage != _T('\0')) {
        ...
        if (!sValue.empty()) {
            if (sItem == _T("file") || sItem == _T("res")) {
                ...
                imageAttribute.rcDest.left = _tcstol(sValue.c_str(), &pstr, 10);
                ASSERT(pstr);
                imageAttribute.rcDest.top = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
                imageAttribute.rcDest.right = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
                imageAttribute.rcDest.bottom = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
            } else if (sItem == _T("source")) {
                imageAttribute.rcSource.left = _tcstol(sValue.c_str(), &pstr, 10);
                ASSERT(pstr);
                imageAttribute.rcSource.top = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
                imageAttribute.rcSource.right = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
                imageAttribute.rcSource.bottom = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
            } else if (sItem == _T("corner")) {
                imageAttribute.rcCorner.left = _tcstol(sValue.c_str(), &pstr, 10);
                ASSERT(pstr);
                imageAttribute.rcCorner.top = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
                imageAttribute.rcCorner.right = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
                imageAttribute.rcCorner.bottom = _tcstol(pstr + 1, &pstr, 10);
                ASSERT(pstr);
...
HWND Window::Create(HWND hwndParent, LPCTSTR pstrName, DWORD dwStyle, DWORD dwExStyle, bool isLayeredWindow, const UiRect& rc) {
    ...
    m_hWnd = ::CreateWindowEx(dwExStyle, className.c_str(), pstrName, dwStyle, rc.left, rc.top, rc.GetWidth(), rc.GetHeight(), hwndParent, NULL,
...
void Window::SetIcon(UINT nRes) {
    HICON hIcon = (HICON)::LoadImage(::GetModuleHandle(NULL), MAKEINTRESOURCE(nRes), IMAGE_ICON, ::GetSystemMetrics(SM_CXICON),
                                     ::GetSystemMetrics(SM_CYICON), LR_DEFAULTCOLOR | LR_SHARED);
    ASSERT(hIcon);
    ::SendMessage(m_hWnd, WM_SETICON, (WPARAM)TRUE, (LPARAM)hIcon);
    hIcon = (HICON)::LoadImage(::GetModuleHandle(NULL), MAKEINTRESOURCE(nRes), IMAGE_ICON, ::GetSystemMetrics(SM_CXSMICON),
...
LRESULT Window::DoHandlMessage(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& handled) {
    ...
    switch (uMsg) {
        ...
        case WM_MOUSEHOVER: {
            ...
            if (m_hwndTooltip == NULL) {
                m_hwndTooltip = ::CreateWindowEx(0, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, CW_USEDEFAULT, CW_USEDEFAULT,
                                                 CW_USEDEFAULT, CW_USEDEFAULT, m_hWnd, NULL, ::GetModuleHandle(NULL), NULL);
        ...
        case WM_MOUSEMOVE: {
            ...
            if (m_pNewHover != NULL && m_pNewHover->GetWindow() != this)
                break;
        ...
        case WM_KILLFOCUS: {
            POINT pt = {GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)};
            m_ptLastMousePos = pt;
            if (m_pEventClick == NULL || typeid(*m_pEventClick) != typeid(ScrollBar))
                break;
            ReleaseCapture();
...
void Window::Paint() {
    ...
    if (m_bIsArranged && m_pRoot->IsArranged() && (m_pRoot->GetFixedWidth() == DUI_LENGTH_AUTO || m_pRoot->GetFixedHeight() == DUI_LENGTH_AUTO)) {
        ...
        if (need_size.cx < m_pRoot->GetMinWidth())
            need_size.cx = m_pRoot->GetMinWidth();
        if (m_pRoot->GetMaxWidth() >= 0 && need_size.cx > m_pRoot->GetMaxWidth())
            need_size.cx = m_pRoot->GetMaxWidth();
        if (need_size.cy < m_pRoot->GetMinHeight())
            need_size.cy = m_pRoot->GetMinHeight();
        if (need_size.cy > m_pRoot->GetMaxHeight())
            need_size.cy = m_pRoot->GetMaxHeight();
        UiRect rect;
    ...
    if (m_hbmpBackground == NULL) {
        ...
        m_hbmpBackground = ::CreateDIBSection(m_hDcPaint, &bmi, DIB_RGB_COLORS, (void**)&m_pBmpBackgroundBits, NULL, 0);
    ...
    if (m_shadow.IsShadowAttached()) {
        ...
        for (int i = newMirrorTop; i < newMirrorBottom; i++) {
            for (int j = newMorrorLeft; j < newMorrorRight; j++) {
                if (!(j < rootPadding.left && i < rootPadding.top) && !(j < rootPadding.left && i >= rootPos.bottom - rootPadding.bottom) &&
    ...
    if (m_bIsLayeredWindow) {
        ...
        ::BitBlt(ps.hdc, rcPaint.left, rcPaint.top, rcPaint.GetWidth(), rcPaint.GetHeight(), m_hDcBackground, rcPaint.left, rcPaint.top, SRCCOPY);
    }
    ::EndPaint(m_hWnd, &ps);
...
void Window::SetFocus(Control* pControl) {
    ...
    if (pControl != NULL && pControl->GetWindow() == this && pControl->IsVisible() && pControl->IsEnabled()) {
        m_pFocus = pControl;
...
Control* CALLBACK Window::__FindControlFromClass(Control* pThis, LPVOID pData) {
    ...
    if ((*pFoundControls)[pFoundControls->size() - 1] != NULL)
        return pThis;
    return NULL;
}
