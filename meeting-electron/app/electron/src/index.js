...
const fs = require('fs');
const { sharingScreen, closeScreenSharingWindow } = require('./sharingScreen');
const {
...
const {
  openMeetingWindow,
  closeMeetingWindow,
} = require('./mainMeetingWindow');
const { watchProtocol } = require('./utils/agreement');
const { addIpcMainListeners } = require('./ipcMain');
const { isLocal, isWin32 } = require('./constant');
...
app.whenReady().then(() => {
  ...
  ipcMain.on('enterRoom', (event, data) => {
    ...
    mainWindow = openMeetingWindow(data);
    // 阻止屏幕休眠
