...
MessageLoop::MessageLoop()
    ...
    if (type_ == kDefaultMessageLoop)
        pump_ = new DefaultMessagePump;
    // else if (type_ == kUVMessageLoop)
    //	pump_ = new LibuvMessagePump;
    g_lazy_ptr.Pointer()->Set(this);

    message_loop_proxy_ = new MessageLoopProxy;
...
// UvMessageLoop* MessageLoop::ToUvMessageLoop()
//{
//	if (type_ == kUVMessageLoop)
//		return reinterpret_cast<UvMessageLoop *>(this);
//	return NULL;
//}
...
void MessageLoop::RunInternal() {
    ...
    if (state_->dispatcher && type() == kUIMessageLoop) {
        static_cast<WinUIMessagePump*>(pump_.get())->RunWithDispatcher(this, state_->dispatcher);
        return;
    }
#endif
...
void MessageLoop::PostTask(Task* task) {
    assert(task);
    PendingTask pending_task(std::bind(&nbase::subtle::TaskClosureAdapter::Run, new nbase::subtle::TaskClosureAdapter(task)));
    AddToIncomingQueue(pending_task);
}

void MessageLoop::PostDelayedTask(Task* task, int64_t delay_ms) {
    assert(task);
    PendingTask pending_task(std::bind(&nbase::subtle::TaskClosureAdapter::Run, new nbase::subtle::TaskClosureAdapter(task)),
                             EvalDelayedRuntime(delay_ms), true);
    AddToIncomingQueue(pending_task);
}

void MessageLoop::PostNonNestableTask(Task* task) {
    assert(task);
    PendingTask pending_task(std::bind(&nbase::subtle::TaskClosureAdapter::Run, new nbase::subtle::TaskClosureAdapter(task)), EvalDelayedRuntime(0),
                             false);
    AddToIncomingQueue(pending_task);
}

void MessageLoop::PostNonNestableDelayedTask(Task* task, int64_t delay_ms) {
    assert(task);
    PendingTask pending_task(std::bind(&nbase::subtle::TaskClosureAdapter::Run, new nbase::subtle::TaskClosureAdapter(task)),
                             EvalDelayedRuntime(delay_ms), false);
    AddToIncomingQueue(pending_task);
}
...
#elif defined(OS_POSIX)
bool IOMessageLoop::WatchFileDescriptor(int fd, bool persistent, Mode mode, FileDescriptorWatcher* controller, Watcher* delegate) {
    assert(this == current());
    return static_cast<IOMessagePump*>(pump())()->WatchFileDescriptor(fd, persistent, static_cast<LibeventMessagePump::Mode>(mode), controller,
                                                                      delegate);
}
#endif
