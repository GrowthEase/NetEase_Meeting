...
const MonitoringSetting: React.FC = () => {
  ...
  const onLocalAudioStatsTimerRef = React.useRef<null | ReturnType<
    typeof setTimeout
  >>()
  const onLocalVideoStatsTimerRef = React.useRef<null | ReturnType<
    typeof setTimeout
  >>()
  const onRemoteAudioStats = React.useRef<null | ReturnType<
    typeof setTimeout
  >>()
  const onRemoteVideoStats = React.useRef<null | ReturnType<
    typeof setTimeout
  >>()
  const [needMemoryWarning, setNeedMemoryWarning] = React.useState(false)

  const [tab, setTab] = React.useState<'overall' | 'av'>('overall')
  const [networkType, setNetworkType] = React.useState<number>(0)
  ...
  useEffect(() => {
    neMeeting?.getNetworkType().then((res) => {
      setNetworkType(res)
    })
  ...
  useEffect(() => {
    ...
    if (tab === 'overall') {
      ...
      getMonitoringInfo()
      // 每 2s 执行一次
      const timer = setInterval(getMonitoringInfo, 2000)
      ...
      <div className="monitoring-content-box-network">
        ...
        <div className="network-content">
          ...
          <div className="content-item">
            ...
            <span>{['', t('ethernet'), t('wifi')][networkType] ?? ''}</span>
          </div>
          <div className="content-item">
            <span className="title">{t('networkType')}</span>
            <span>{['', 'LAN', 'WLAN'][networkType] ?? ''}</span>
          </div>
        </div>
      </div>
    )
