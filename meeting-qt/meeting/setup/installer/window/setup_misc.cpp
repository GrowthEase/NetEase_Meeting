    ...
    if (res != SZ_OK) {
        ...
        for (i = 0; i < iFileNum && !destroy_wnd_; i++) {
            ...
            if (f->IsDir) {
                ...
                res = SzArEx_Extract(&db, &lookStream.s, i, &blockIndex, &outBuffer, &outBufferSize, &offset, &outSizeProcessed, &allocImp,
                                     &allocTempImp);
                if (res != SZ_OK) {
...
void SetupForm::WriteRegList() {
    if (!destroy_wnd_) {
        ...
        pre_progress_pos_ = 100 - PROGRESS_COPY_HISTORICALDATA_REG;
        LOG_APP("WriteRegList end");

        LOG_APP("CopyHistoricalData begin");
        CopyHistoricalData();
        SetProgressCurStepPos(PROGRESS_COPY_HISTORICALDATA_REG);
        pre_progress_pos_ = 100;
        LOG_APP("CopyHistoricalData end");
...
// 拷贝历史数据
void SetupForm::CopyHistoricalData() {
    std::vector<std::wstring> historicalData{_T("config"), _T("image")};
    for (auto& it : historicalData) {
        std::wstring instalDir = CSetupData::GetInstDir();
        std::wstring preInstalDir = CSetupData::GetLastInstDir() + it;
        if (instalDir != preInstalDir) {
            SHFILEOPSTRUCTW s = {0};
            s.hwnd = m_hWnd;
            s.wFunc = FO_COPY;
            s.fFlags = FOF_SILENT | FOF_NOCONFIRMMKDIR | FOF_NOCONFIRMATION | FOF_NOERRORUI | FOF_NO_UI;
            s.pTo = instalDir.c_str();
            s.pFrom = preInstalDir.c_str();
            SHFileOperationW(&s);
        }
    }
}
