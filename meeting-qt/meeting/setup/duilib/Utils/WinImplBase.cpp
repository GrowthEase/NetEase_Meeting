...
LRESULT WindowImplBase::OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
        ...
        if (pt.y < rcClient.top + rcSizeBox.top) {
            if (pt.y >= rcClient.top) {
                if (pt.x < rcClient.left + rcSizeBox.left)
                    return HTTOPLEFT;
                else if (pt.x > rcClient.right - rcSizeBox.right)
                    return HTTOPRIGHT;
                else
                    return HTTOP;
            } else
    ...
    if (pt.x >= rcClient.left + rcCaption.left && pt.x < rcClient.right - rcCaption.right && pt.y >= rcClient.top + rcCaption.top &&
        pt.y < rcClient.top + rcCaption.bottom) {
...
LRESULT WindowImplBase::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
    ...
    if (pRoot->GetFixedWidth() == DUI_LENGTH_AUTO || pRoot->GetFixedHeight() == DUI_LENGTH_AUTO) {
        ...
        if (need_size.cx < pRoot->GetMinWidth())
            need_size.cx = pRoot->GetMinWidth();
        if (pRoot->GetMaxWidth() >= 0 && need_size.cx > pRoot->GetMaxWidth())
            need_size.cx = pRoot->GetMaxWidth();
        if (need_size.cy < pRoot->GetMinHeight())
            need_size.cy = pRoot->GetMinHeight();
        if (need_size.cy > pRoot->GetMaxHeight())
            need_size.cy = pRoot->GetMaxHeight();
