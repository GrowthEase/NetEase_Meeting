 ...
 *	Brief		an implemention of observer list
 *
 */
...
/*
 * Ҫһ⣺
 * MessagePumpڱObserverListʱ
 * һObserverڲRemoveObserverƳԼ߱Observerᵼ½ıiteratorerase
 *
 * Google Chromeiteratorlazy eraseʾ룺
 *
...
public:
    ...
    int ReleaseLazyErase() {
        assert(lazy_erase_ > 0);
        return --lazy_erase_;
    }
...
public:
    AutoLazyEraser(LazyEraser* lazy_eraser)
        : ptr_(lazy_eraser) {
        if (ptr_)
