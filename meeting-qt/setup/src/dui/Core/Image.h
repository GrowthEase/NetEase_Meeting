...
public:
    ...
    void SetPropertyItem(Gdiplus::PropertyItem* propertyItem) { m_propertyItem.reset(propertyItem); }

    void PushBackHBitmap(HBITMAP hBitmap) { m_vecBitmap.push_back(hBitmap); }

    HBITMAP GetHBitmap(int index) { return m_vecBitmap[index]; }

    int GetFrameCount() { return m_vecBitmap.size(); }

    int IsGif() { return m_vecBitmap.size() > 1; }
...
public:
    ...
    void SetCurrentFrame(int iCurrentFrame) { m_iCurrentFrame = iCurrentFrame; }

    bool IsPlaying() { return m_bPlaying; }

    void SetPlaying(bool playing) { m_bPlaying = playing; }

    HBITMAP GetCurrentHBitmap() { return imageCache->GetHBitmap(m_iCurrentFrame); }

    int GetCurrentInterval() { return imageCache->GetInterval(m_iCurrentFrame); }
