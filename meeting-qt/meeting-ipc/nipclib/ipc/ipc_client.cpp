...
void IPCClient::OnSocketError(int error_code, const void* data, size_t size) {
...
void IPCClient::OnReceive(const IPCData& data) {
    if (Ready()) {
        ...
        if (data->compare("server ack") == 0) {
            ...
            Log(1, std::string("transport ready"));
            if (Ready()) {
                Log(1, std::string("Ready()."));
                return;
            }
            SetReady();
            keep_alive_latest_ = std::chrono::steady_clock::now();
