...
void LibeventMessagePump::FileDescriptorWatcher::OnFileCanReadWithoutBlocking(int fd, LibeventMessagePump* pump) {
    pump->PreProcessIOEvent();
    watcher_->OnFileCanReadWithoutBlocking(fd);
    pump->PostProcessIOEvent();
}

void LibeventMessagePump::FileDescriptorWatcher::OnFileCanWriteWithoutBlocking(int fd, LibeventMessagePump* pump) {
    pump->PreProcessIOEvent();
...
LibeventMessagePump::~LibeventMessagePump() {
    ...
    if (wakeup_pipe_in_ >= 0) {
        ...
        if (r != 0) {
            DEFLOG(LogInterface::LV_ERR, __FILE__, __LINE__, "LibeventMessagePump close wakeup_pipe_in_ error:%d", errno);
        }
    }
    if (wakeup_pipe_out_ >= 0) {
        int r = ::close(wakeup_pipe_out_);
        if (r != 0) {
            DEFLOG(LogInterface::LV_ERR, __FILE__, __LINE__, "LibeventMessagePump close wakeup_pipe_out_ error:%d", errno);
        }
    }
    if (event_base_)
...
bool LibeventMessagePump::WatchFileDescriptor(int fd, bool persistent, Mode mode, FileDescriptorWatcher* controller, Watcher* delegate) {
    if (fd <= 0)
        assert(0);
...
void LibeventMessagePump::ScheduleWork() {
    ...
    if (!(write_len == 1 || errno == EAGAIN)) {
        DEFLOG(LogInterface::LV_ERR, __FILE__, __LINE__, "LibeventMessagePump ScheduleWork write errno %d", errno);
    }
}
...
bool LibeventMessagePump::Init() {
    int fds[2];
    if (pipe(fds)) {
        DEFLOG(LogInterface::LV_ERR, __FILE__, __LINE__, "pipe() failed, errno:%d", errno);
        return false;
    }
    if (SetNonBlocking(fds[0])) {
        DEFLOG(LogInterface::LV_ERR, __FILE__, __LINE__, "set_block for pipe fd[0] failed, errno:%d", errno);
        return false;
    }
    if (SetNonBlocking(fds[1])) {
        DEFLOG(LogInterface::LV_ERR, __FILE__, __LINE__, "set_block for pipe fd[1] failed, errno:%d", errno);
        return false;
    }
...
void LibeventMessagePump::OnWakeup(int socket, short flags, void* context) {
    nbase::LibeventMessagePump* that = static_cast<nbase::LibeventMessagePump*>(context);
