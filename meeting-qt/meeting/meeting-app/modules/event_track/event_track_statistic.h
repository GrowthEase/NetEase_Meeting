...
public:
    virtual std::string EventTag() const override { return event_tag_; }
    virtual std::string EventType() const override { return event_type_; }
    ...
    public:
        ...
        ~Reportor() {
            ...
            if (_this->timer_.fun_start_timer != nullptr) {
                auto* imp = _this;
                _this->timer_.fun_start_timer(_this->cache_time_block_, [imp]() { Reportor(imp).Report(); });
            }
        }
...
public:
    ...
    virtual void SetAsyncTaskRunner(const typename ParentType::AsyncTaskRunner& runner) override { async_task_runner_ = runner; }
    virtual void SetRequestURLGettor(const typename ParentType::RequestURLGettor& url_gettor) override { url_gettor_ = url_gettor; }
    virtual void SetHttpRequestor(const typename ParentType::HttpRequestor& requestor) override { http_requestor_ = requestor; }
    virtual void SetHttpRequestHeadFiller(const typename ParentType::HttpRequestHeadFiller& filler) override { http_request_head_filler_ = filler; }
    virtual void SetTimer(typename ParentType::Timer timer) override { timer_ = timer; }
    ...
    virtual void AttachStop(const typename ParentType::EventTrackStatisticClosure& stop_callback) override { stop_callback_ = stop_callback; }
...
private:
    void Report(const TEventFromatObject& format_object) {
        auto task = [this, format_object]() {
            typename ParentType::HttpRequest request =
                std::make_shared<typename ParentType::HttpRequestInfo>(url_gettor_ == nullptr ? "" : url_gettor_());
