...
public:
    ...
    Pack(PackBuffer& pb, size_t off = 0, int bo = 0)
        : buffer_(pb)
        , byte_order_(bo) {
        offset_ = pb.size() + off;
    ...
    Pack& push(const void* s, size_t n) {
        buffer_.append((const char*)s, n);
        return *this;
    }
    Pack& push(const void* s) {
        buffer_.append((const char*)s);
        return *this;
    }

    Pack& push_bool(bool b) {
        uint8_t u8 = (b ? 1 : 0);
        return push(&u8, 1);
    }
...
public:
    ...
    std::string pop_fetch(size_t k) const { return std::string(pop_fetch_ptr(k), k); }
...
struct Marshallable {
    ...
    virtual std::ostream& trace(std::ostream& os) const { return os << "trace Marshallable [ not immplement ]"; }
};
