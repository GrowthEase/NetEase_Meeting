...
public:
    ...
    IPCAsyncResponseCallback(const TCallback& cb)
        : callback_(cb) {}
...
private:
    inline void RemoveResponseCallback_i(uint64_t sn) { request_list_.erase(sn); }
    inline void AddResponseCallback_i(uint64_t sn, const IPCAsyncResponseCallback& cb) { request_list_[sn] = cb; }
...
public:
    IService(int sid)
        : sid_(sid)
        , ipc_controller_(nullptr)
        , proc_thread_(nullptr){};
    ...
    virtual void OnReceivePack(int cid, const std::string& data, const IPCAsyncResponseCallback& cb) {
        if (proc_thread_ != nullptr)
            proc_thread_->TaskLoop()->PostTask(ToWeakCallback([this, cid, data, cb]() { OnPack(cid, data, cb); }));
    }
    virtual void OnReceivePack(int cid, const std::string& data, uint64_t sn) {
        if (proc_thread_ != nullptr)
            proc_thread_->TaskLoop()->PostTask(ToWeakCallback([this, cid, data, sn]() { OnPack(cid, data, sn); }));
    }
    void SetProcThread(const std::shared_ptr<NS_NIPCLIB::IPCThread>& thread) { proc_thread_ = thread; }
    void SetIPCController(const std::shared_ptr<TIPCType>& ipc_controller) { ipc_controller_ = ipc_controller; }
...
protected:
    ...
    virtual void SendData(int cid, const NEMIPCProtocolBody& data, const IPCAsyncResponseCallback& cb) { SendData(GetSID(), cid, data, cb); }
    virtual void SendData(int cid, const NEMIPCProtocolBody& data, uint64_t sn) { SendData(GetSID(), cid, data, sn); }
