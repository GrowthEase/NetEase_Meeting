...
MoreItemManager::MoreItemManager(QObject* parent)
    ...
    connect(FeedbackManager::getInstance(), &FeedbackManager::uploadStatusChanged,
            [this](bool uploading) { modifyItem(101, "", uploading ? 2 : 1); });
}
...
int MoreItemManager::itemCountMore_S() const {
    ...
    index = items.indexOf(MoreItem(kWhiteboardMenuId));
    if (-1 != index) {
        items.remove(index);
    }
