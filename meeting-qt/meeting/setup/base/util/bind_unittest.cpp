...
TEST(BindTest, ArgumentBinding) {
    ...
    Callback<int*(void)> bind_primitive_pointer_cb = Bind(&PolymorphicIdentity<int*>, &n);
    ...
    Callback<const char*(void)> bind_string_literal_cb = Bind(&CStringIdentity, "hi");
    EXPECT_STREQ("hi", bind_string_literal_cb.Run());

    Callback<int(void)> bind_template_function_cb = Bind(&PolymorphicIdentity<int>, 4);
    ...
    c.value = 7;
    Callback<int(void)> bind_pointer_promotes_cb = Bind(&UnwrapNoRefParentPtr, &c);
    EXPECT_EQ(7, bind_pointer_promotes_cb.Run());

    c.value = 8;
    Callback<int(void)> bind_const_reference_promotes_cb = Bind(&UnwrapNoRefParentConstRef, c);
...
TEST(BindTest, UnboundArgumentTypeSupport) {
    ...
    Callback<void(const int&)> unbound_const_ref_cb = Bind(&VoidPolymorphic1<const int&>);
    Callback<void(int[])> unbound_unsized_array_cb = Bind(&VoidPolymorphic1<int[]>);
    Callback<void(int[2])> unbound_sized_array_cb = Bind(&VoidPolymorphic1<int[2]>);
    Callback<void(int[][2])> unbound_array_of_arrays_cb = Bind(&VoidPolymorphic1<int[][2]>);
}
...
TEST(BindTest, ConstRef) {
    ...
    Callback<int(void)> all_const_ref_cb = Bind(&GetCopies, ConstRef(counter));
...
TEST(BindTest, ArgumentCopies) {
    ...
    Callback<void(void)> copy_cb = Bind(&VoidPolymorphic1<CopyCounter>, counter);
    EXPECT_GE(1, copies);
    EXPECT_EQ(0, assigns);

    copies = 0;
    assigns = 0;
    Callback<void(CopyCounter)> forward_cb = Bind(&VoidPolymorphic1<CopyCounter>);
    ...
    Callback<void(CopyCounter)> coerce_cb = Bind(&VoidPolymorphic1<CopyCounter>);
