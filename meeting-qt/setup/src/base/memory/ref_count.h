...
public:
    ...
    inline int32_t ref() const { return ref_; }
...
public:
    ...
    // destructor function
    virtual ~ThreadSafeRefCountBase() { assert(ref_ == 0); }

    // Get the current count of the object reference
    inline base::subtle::Atomic32 ref() const { return base::subtle::Acquire_Load(&ref_); }
    ...
    // increase the count of the object reference
    inline base::subtle::Atomic32 AddRef() const { return base::subtle::NoBarrier_AtomicIncrement(&ref_, 1); }
...
struct DefaultRefCountedThreadSafeTraits {
    static void Destruct(const T* x) {
        ...
        ThreadSafeRefCountT<T, DefaultRefCountedThreadSafeTraits>::DeleteInternal(x);
    }
};
...
class BASE_EXPORT ThreadSafeRefCount : public ThreadSafeRefCountT<ThreadSafeRefCount> {
public:
};
...
public:
    ...
    inline bool is_nil() const { return (ptr_ == NULL); }
