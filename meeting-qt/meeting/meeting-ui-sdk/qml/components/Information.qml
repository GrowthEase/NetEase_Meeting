...
Popup {
    ...
    onVisibleChanged: {
        if(visible) {
            ...
            inviteRow.visible = meetingInviteUrl !== ''
        }
    }
    ...
    onClosed: {
        idMeetingInfo.closePolicy = Popup.CloseOnEscape | Popup.CloseOnPressOutside
    }
    ...
    ColumnLayout {
        ...
        RowLayout {
            ...
            visible: meetingManager.meetingIdDisplayOption === 1 ||
                     ...
                     (meetingManager.meetingIdDisplayOption === MeetingStatus.DISPLAY_SHORT_ID_ONLY && meetingManager.shortMeetingNum === '')
            Label {
        ...
        RowLayout {
            ...
            Image {
                ...
                mipmap: true
                MouseArea {
                    id: meetingInviteUrlCopyBtn
                    anchors.fill: parent
                    onClicked: {
                        clipboard.setText(meetingInviteUrl)
                        toast.show(qsTr('Invite Url has been copied'))
                    }
                }
                Accessible.role: Accessible.Button
                Accessible.name: "meetingInviteUrl"
                Accessible.onPressAction: if (enabled) meetingInviteUrlCopyBtn.clicked(Qt.LeftButton)
            }
        }
    }
}
