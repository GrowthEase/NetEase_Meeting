...
#include "base/framework/task.h"  // for Task
#if defined(OS_WIN)
...
#endif  // OS_WIN
//#include "base/framework/libuv_message_pump.h"		// for LibuvMessagePump
#include "base/forbid_copy.h"
#include "base/memory/ref_count.h"      // for scoped_refptr, etc.
...
public:
    ...
    // 下面的PostTask族函数已经作废，请改用Closure形式的PostTask族函数！！！
    //
    ...
    void PostTask(Task* task);                                      // 作废
    void PostDelayedTask(Task* task, int64_t delay_ms);             // 作废
    void PostNonNestableTask(Task* task);                           // 作废
    void PostNonNestableDelayedTask(Task* task, int64_t delay_ms);  // 作废
...
protected:
    ...
    struct PendingTask {
        ...
        PendingTask(const StdClosure& task, nbase::TimeTicks delayed_run_time, bool nestable);
...
public:
    ...
    enum Mode{WATCH_READ = LibeventMessagePump::WATCH_READ, WATCH_WRITE = LibeventMessagePump::WATCH_WRITE,
...
// class BASE_EXPORT UvMessageLoop : public MessageLoop
//{
// public:
...
//	{
//		return reinterpret_cast<UvMessageLoop *>(MessageLoop::current());
//	}
//
//	virtual void AddToIncomingQueue(const PendingTask &task);
//
