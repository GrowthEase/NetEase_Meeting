    ...
    switch (cid) {
        ...
        case MettingCID::MettingCID_ScheduleMeeting: {
            ...
            if (request.Parse(data)) {
                scheduleMeeting(request.param_,
                                ToWeakCallback([this, sn](NEErrorCode error_code, const std::string& error_msg, const NEMeetingItem& item) {
        ...
        case MettingCID::MettingCID_CancelMeeting: {
            PreMeetingRequest request;
            if (request.Parse(data)) {
                cancelMeeting(request.param_.meetingId, ToWeakCallback([this, sn](NEErrorCode error_code, const std::string& error_msg) {
                                  PreMeetingResponse response;
                                  response.error_code_ = error_code;
                                  response.error_msg_ = error_msg;
        ...
        case MettingCID::MettingCID_DeleteMeeting: {
            ...
            //         if (request.Parse(data))
            //         {
            //             deleteMeeting(request.param_.meetingUniqueId, ToWeakCallback([this, sn](NEErrorCode error_code, const std::string&
            //             error_msg) {
            //                 PreMeetingResponse response;
            //                 response.error_code_ = error_code;
            //                 response.error_msg_ = error_msg;
        ...
        case MettingCID::MettingCID_GetMeetingItemById: {
            PreMeetingRequest request;
            if (request.Parse(data)) {
                getMeetingItemById(request.param_.meetingId,
                                   ToWeakCallback([this, sn](NEErrorCode error_code, const std::string& error_msg, const NEMeetingItem& item) {
                                       PreMeetingResponse response;
                                       response.error_code_ = error_code;
                                       response.error_msg_ = error_msg;
        ...
        case MettingCID::MettingCID_GetMeetingList: {
            ...
            if (request.Parse(data)) {
                getMeetingList(request.params_, ToWeakCallback([this, sn](NEErrorCode error_code, const std::string& error_msg,
                                                                          const std::list<NEMeetingItem>& packInfo) {
