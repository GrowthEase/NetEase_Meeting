...
void WinIOMessagePump::ScheduleWork() {
    ...
    BOOL ret = ::PostQueuedCompletionStatus(port_, 0, reinterpret_cast<ULONG_PTR>(this), reinterpret_cast<OVERLAPPED*>(this));
    assert(ret);
}
...
bool WinIOMessagePump::GetIOItem(DWORD timeout, IOItem* item) {
    ...
    if (!::GetQueuedCompletionStatus(port_.Get(), &item->bytes_transfered, &key, &overlapped, timeout)) {
        if (!overlapped)
...
bool WinIOMessagePump::ProcessInternalIOItem(const IOItem& item) {
    if (this == reinterpret_cast<WinIOMessagePump*>(item.context) && this == reinterpret_cast<WinIOMessagePump*>(item.handler)) {
